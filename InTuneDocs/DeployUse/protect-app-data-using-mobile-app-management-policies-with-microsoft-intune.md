...
-   **Устройства, управляемые и регистрируемые в стороннем решении по управлению мобильными устройствами**.   Обычно это принадлежащие компании устройства.
...
- Политики перемещения данных, такие как
- **Запретить операцию "Сохранить как"**, **Ограничить операции "вырезать", "копировать" и "вставить"**.

### Параметры политик доступа, такие как **Требовать простой ПИН-код для доступа**, **Блокировать запуск управляемых приложений на устройствах со снятой защитой или с корневым доступом**.

![Защита данных с помощью политик MAM на устройствах под управлением решения MDM:](../media/MAM_BYOD_November.png)

**Изображение, демонстрирующее, как работают политики MAM на устройствах BYOD**-
...
![Защита данных с помощью политик MAM для устройств без регистрации](../media/MAM_ManagedDevices_November.png)

Изображение, демонстрирующее, как работают политики MAM на управляемых устройствах.
...
-   Однако существуют некоторые ограничения, которые следует учитывать:  Вы не можете развертывать приложения на устройстве.

-   Конечный пользователь должен получать приложения из Магазина.
...
Приложения, поддерживающие множественную идентификацию, позволяют использовать разные учетные записи — рабочую и личную — для доступа к одним и тем же приложениям, тогда как политики MAM применяются только к приложениям в рабочем контексте. Например, когда пользователь запускает приложение OneDrive с помощью своей рабочей учетной записи, он не может перемещать файлы в личное хранилище.  

Однако когда пользователь открывает OneDrive с помощью личной учетной записи, он может копировать и перемещать данные из своего личного хранилища OneDrive без ограничений.
...
[Дальнейшие шаги](get-ready-to-configure-mobile-app-management-policies-with-microsoft-intune.md)

[Подготовка к настройке политик управления мобильными приложениями](create-and-deploy-mobile-app-management-policies-with-microsoft-intune.md)
