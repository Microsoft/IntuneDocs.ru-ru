...
5.  **Добавление политики регистрации корпоративных устройств** В [консоли администрирования Microsoft Intune](http://manage.microsoft.com) перейдите в меню **Политика** &gt; **Регистрация корпоративных устройств** и щелкните **Добавить**.
    ...
    Затем включите параметр **Настройка параметров программы регистрации устройств для этой политики** для поддержки DEP.
     ...
     - **Параметры помощника установки** — эти параметры необязательны, и их можно настроить позже в меню **Параметры** iOS.
        ...
        - **Terms and Conditions** (Условия и положения) — если параметр включен, помощник по настройке предложит пользователю принять условия и положения Apple во время активации. - **Touch ID** — если параметр включен, помощник по настройке будет запрашивать параметры этой службы во время активации. - **Apple Pay** — если параметр включен, помощник по настройке будет запрашивать параметры этой службы во время активации. - **Zoom** — если параметр включен, помощник по настройке будет запрашивать параметры этой службы во время активации. - **Siri** — если параметр включен, помощник по настройке будет запрашивать параметры этой службы во время активации. - **Send diagnostic data to Apple** (Отправлять диагностические данные в Apple) — если параметр включен, помощник по настройке будет запрашивать параметры этой службы во время активации -  **Enable additional Apple Configurator management** (Включить дополнительное управление Apple Configurator) —установите этот параметр в значение **Запретить**, чтобы не выполнять синхронизацию файлов с iTunes и управление ими через Apple Configurator. Корпорация Майкрософт рекомендует установить этот параметр в значение **Запретить**, экспортировать все дальнейшие настройки из Apple Configurator и затем выполнить развертывание в виде настраиваемого профиля конфигурации iOS через Intune вместо того, чтобы разрешать развертывание вручную с сертификатом или без него с помощью этого параметра.
        - **Disallow** (Запретить) — запрещает устройству обмен данными через USB (отключает связывание). - **Allow** (Разрешить) — позволяет устройству обмениваться данными через подключение USB с любым компьютером ПК или Mac. - **Require certificate** (Требовать сертификат) — разрешает связывание с компьютером Mac с сертификатом, импортированным в профиль регистрации.
