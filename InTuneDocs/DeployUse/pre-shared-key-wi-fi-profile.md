...
3.  Добавление нового параметра OMA URI
   ...
   г.   **OMA-URI**: ./Vendor/MSFT/Wi-Fi /Profile/<SSID>/Settings

Примечание. В начале обязательно должна стоять точка.

SSID — это SSID, для которого создается политика. Например,
`./Vendor/MSFT/Wi-Fi/Profile/Hotspot-1/Settings`

  д.    Поле значения: это поле для вставки XML-кода. Вот пример. Каждое значение должно быть адаптировано для параметров сети. Некоторые сведения приведены в разделе комментариев кода.
...
ПРИМЕЧАНИЕ. Эта политика может быть развернута только для групп пользователей.
...
1.     На компьютере, который подключен или недавно был подключен к беспроводной сети, откройте следующую папку: C:\ProgramData\Microsoft\Wlansvc\Profiles\Interfaces\{guid}. Рекомендуется использовать компьютер, подключенный к небольшому числу беспроводных сетей, так как вам придется просмотреть каждый профиль, чтобы найти нужный.
