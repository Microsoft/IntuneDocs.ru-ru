...
## Укажите параметры политики для брандмауэра Windows
...
### Настройка предопределенных исключений
...
|**Кэширование членов одноранговой группы BITS**|Позволяет клиентам использовать фоновую интеллектуальную службу передачи (BITS) для поиска и совместного использования файлов, хранящихся в кэше BITS на клиентах в той же подсети. Этот параметр использует веб-службы на устройствах (WSDAPI) и удаленный вызов процедур (RPC).|
...
|**Ловушка SNMP**|Позволяет управляемым компьютерам получать сетевой трафик из службы ловушек SNMP.|
