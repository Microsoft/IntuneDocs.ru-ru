---
title: "Руководство для разработчиков по пакету SDK для приложений Microsoft Intune в iOS | Документы Майкрософт"
...
Пакет SDK приложений Microsoft Intune для iOS позволяет встроить в ваше собственное приложение для iOS политики защиты приложений, также известные как политики **APP** или **MAM**. В приложение, которое использует MAM, встроен пакет SDK для приложений Intune. ИТ-администраторы могут развертывать политики защиты приложений в вашем мобильном приложении, когда Intune активно управляет им.

## <a name="prerequisites"></a>Предварительные требования

* Потребуется компьютер под управлением Mac OS X 10.8.5 или более поздней версии с установленным XCode версии 8 или более поздней.

* Ваше приложение должно быть предназначено для iOS 9 или более поздней версии.
...
1. **Вариант 1 (рекомендуется)**: связывание `IntuneMAM.framework` с проектом. Перетащите библиотеку `IntuneMAM.framework` в список **Linked Frameworks and Libraries** (Связанные платформы и библиотеки) цели проекта.

    > [!NOTE]
    > При использовании платформы вам необходимо вручную убрать архитектуры симулятора из универсальной платформы перед отправкой приложения в App Store. В разделе [отправки приложения в магазине приложений](#Submit-your-app-to-the-App-Store) подробнее.

2. **Вариант 2**: связывание с библиотекой `libIntuneMAM.a`. Перетащите библиотеку `libIntuneMAM.a` в список **Linked Frameworks and Libraries** (Связанные платформы и библиотеки) цели проекта.
...
3. Добавьте в проект следующие платформы iOS.
    * MessageUI.framework
    * Security.framework
...
4. Добавьте пакет ресурсов `IntuneMAMResources.bundle` в проект, перетащив этот набор в узел **Copy Bundle Resources** (Копирование ресурсов пакета) раздела **Build Phases** (Этапы сборки).

    ![Пакет SDK для приложений Intune для iOS — копирование пакетов ресурсов](../media/intune-app-sdk-ios-copy-bundle-resources.png)

5. Если ваше мобильное приложение определяет главный NIB- или STORYBOARD-файл в своем файле Info.plist, удалите поля **Main Storyboard** или **Main Nib**. В Info.plist добавьте эти поля и их соответствующие значения в новый словарь **IntuneMAMSettings** со следующими именами ключей, если применимо.
    * MainStoryboardFile
  ...
  > Если ваше мобильное приложение не определяет главный NIB- или STORYBOARD-файл в своем Info.plist, эти параметры не являются обязательными.
    ...
    Можно просмотреть Info.plist в необработанном формате (чтобы увидеть названия ключей), щелкнув правой кнопкой мыши в любом месте основного текста документа и изменив тип представления на **Show Raw Keys/Values** (Показать необработанные ключи и (или) значения).
...
7. После включения совместного использования цепочки ключей сделайте следующее для создания отдельной группы доступа, в которой будут храниться данные пакета SDK для приложений Intune. Группу доступа к цепочке ключей можно создать с помощью пользовательского интерфейса или с помощью файла прав. При использовании пользовательского интерфейса для создания группы доступа к цепочке ключей обязательно выполните следующие действия.
    ...
    3. Добавьте `com.microsoft.adalcache` в свои существующие группы доступа.
        ...
        4. Добавьте `com.microsoft.workplacejoin` в свои существующие группы доступа.
            ![Пакет SDK для приложений Intune (iOS) — общий доступ к цепочке ключей](../media/intune-app-sdk-ios-keychain-sharing.png)

      5. Если для создания группы доступа к цепочке ключей вы используете файл прав, добавьте `$(AppIdentifierPrefix)` в начало этой группы в файле прав. Пример.
...
Если ваше приложение использует ADAL для проверки подлинности пользователей, оно должно применить существующие значения регистрации и переопределить значения пакета SDK для приложений Intune по умолчанию. Это гарантирует, что пользователей не попросят пройти проверку подлинности дважды (один раз в пакете SDK для приложений Intune, а другой — в приложении).

### <a name="recommendations"></a>Рекомендации

Рекомендуется связать приложение с [последней версии ADAL](https://github.com/AzureAD/azure-activedirectory-library-for-objc/releases) в главной ветви. Сейчас пакет SDK для приложений Intune использует ветвь брокера ADAL для обеспечения работы приложений, требующих условного доступа. (Таким образом, эти приложения зависят от приложения Microsoft Authenticator.) Тем не менее пакет SDK по-прежнему совместим с главной ветвью ADAL. Используйте ту ветвь, которая лучше подходит для вашего приложения.
...
1. Скачайте [библиотеку проверки подлинности для Azure Active Directory (ADAL) для Objective-C](https://github.com/AzureAD/azure-activedirectory-library-for-objc) с сайта GitHub, а затем выполните [инструкции](https://github.com/AzureAD/azure-activedirectory-library-for-objc/blob/master/README.md) по скачиванию ADAL с помощью подмодулей Git или CocoaPods.

2. Включите пакет ресурсов `ADALiOSBundle.bundle` в проект, перетащив его в узел **Copy Bundle Resources** (Копирование ресурсов пакета) раздела **Build Phases** (Этапы сборки).

3. Добавьте `-force_load {PATH_TO_LIB}/libADALiOS.a` в параметр конфигурации сборки `OTHER_LDFLAGS` проекта или в **Other Linker Flags** (Другие флаги компоновщика) в пользовательском интерфейсе. `PATH_TO_LIB` следует заменить на расположение двоичных файлов ADAL.



### <a name="share-the-adal-token-cache-with-other-apps-signed-with-the-same-provisioning-profile"></a>Как использовать кэш маркера ADAL совместно с другими приложениями, подписанными при помощи одного и того же профиля подготовки? **
...
2. Включите единый вход ADAL, добавив группы доступа `com.microsoft.adalcache` и `com.microsoft.workplacejoin` к правам цепочки ключей.

3. Если вы явным образом задаете группу цепочки ключей для общего кэша ADAL, убедитесь, что она имеет значение `<app_id_prefix>.com.microsoft.adalcache`. ADAL автоматически задает это значение, пока вы не переопределите его. Если вы хотите задать настраиваемую группу цепочки ключей для замены `com.microsoft.adalcache`, укажите ее в разделе "IntuneMAMSettings" файла Info.plist, используя ключ `ADALCacheKeychainGroupOverride`.

### <a name="configure-adal-settings-for-the-intune-app-sdk"></a>Настройка параметров ADAL для пакета SDK для приложений Intune

Если приложение уже использует ADAL для проверки подлинности и имеет свои собственные параметры ADAL, можно настроить принудительное использование пакетом SDK для приложений Intune тех же параметров во время проверки подлинности в Azure Active Directory. Это гарантирует, что приложение не будет дважды отображать пользователю запрос на проверку подлинности. Сведения об указании значений следующих параметров см. в разделе [Настройка параметров пакета SDK для приложений Intune](#configure-settings-for-the-intune-app-sdk).  
...
Если ваше приложение уже использует ADAL, требуются следующие настройки.
...
Кроме того, можно переопределить URL-адрес центра сертификации Azure AD URL-адресом конкретного клиента во время выполнения. Чтобы сделать это, просто определите свойство `aadAuthorityUriOverride` для экземпляра `IntuneMAMPolicyManager`.

> [!NOTE]
> Настройка URL-адрес центра сертификации AAD требуется при [использовании политики защиты приложений без регистрации устройств](#App-protection-policy-without-device-enrollment), чтобы позволить пакету SDK многократно задействовать маркер обновления ADAL, полученный приложением.

Пакет SDK будет продолжать использовать этот URL-адрес Центра для обновления политики и любых последующих запросов на регистрацию, пока это значение не будет очищено или изменено.  Поэтому очень важно очищать значение, когда управляемый пользователь выходит из приложения, и сбрасывать это значение при входе нового управляемого пользователя.

### <a name="if-your-app-does-not-use-adal"></a>Если приложение не использует ADAL

Если приложение не использует ADAL, пакет SDK для приложений Intune предоставит значения по умолчанию для параметров ADAL и будет обрабатывать операции проверки подлинности в Azure AD. Вам не нужно указывать значения для параметров ADAL, перечисленные выше.

## <a name="app-protection-policy-without-device-enrollment"></a>Политика защиты приложений без регистрации устройства

### <a name="overview"></a>Обзор
Политика защиты приложений Intune без регистрации устройств, также известная как **APP-WE** или MAM-WE, позволяет управлять приложениями с помощью Intune без необходимости регистрации устройств в управлении мобильными устройствами Intune (MDM). Чтобы поддерживать эти новые функции, приложения должны быть включены в зарегистрированные учетные записи пользователей для управления устройствами. Чтобы использовать новые API-интерфейсы, сделайте следующее.
...
### <a name="register-user-accounts"></a>Регистрация учетных записей пользователей

Приложение может получать политики защиты приложений из службы Intune, если оно зарегистрировано в службе APP-WE от имени учетной записи указанного пользователя. Приложение отвечает за регистрацию любого вошедшего в систему пользователя с использованием пакета SDK. После проверки подлинности учетной записи нового пользователя приложение должно вызвать метод `registerAndEnrollAccount` из Headers/IntuneMAMEnrollment.h:
...
### <a name="deregister-user-accounts"></a>Отмена регистраций учетных записей пользователей
...
Этот метод должен вызываться до удаления токенов Azure AD учетной записи пользователя. Пакету SDK требуется, чтобы токен AAD учетной записи пользователя выполнял определенные запросы к службе APP-WE от имени пользователя.
...
Вызвав этот метод, пакет SDK запросит учетные данные пользователя, если не удастся найти токен. Пакет SDK попытается зарегистрировать приложение с помощью службы APP-WE от имени указанной учетной записи пользователя. Этот метод можно вызывать с "nil" в качестве удостоверения. В этом случае пакет SDK регистрируется на устройстве при помощи существующего управляемого пользователя, а при отсутствии такового запрашивает имя пользователя.
...
Уведомления передаются через методы делегата в `Headers/IntuneMAMEnrollmentDelegate.h`:
...
Этот объект определяется в `IntuneMAMEnrollmentStatus.h` вместе с конкретными кодами состояния, которые могут возвратиться.
...
### <a name="get-app-protection-policy"></a>Получение политики защиты приложений
...
Словарь **IntuneMAMSettings** в файле Info.plist приложения можно использовать для настройки пакета SDK для приложений Intune. Если словарь IntuneMAMSettings не обнаружен в файле Info.plist, в файле Info.plist приложения необходимо создать словарь с именем поля IntuneMAMSettings.

В словаре IntuneMAMSettings можно добавить строки типа "ключ-значение" с параметрами конфигурации для настройки пакета SDK. В таблице ниже приведен список всех поддерживаемых параметров. В таблице ниже приведен список всех поддерживаемых параметров.
...
MAMPolicyRequired| Логическое значение| Указывает, будет ли блокироваться запуск приложения при отсутствии политики защиты приложений Intune. По умолчанию используется значение NO (Нет). <br><br> Примечание. В магазин приложений невозможно отправить приложения, у которых параметр MAMPolicyRequired имеет значение YES (Да). | Необязательный параметр. |
...
В любой момент времени каждый поток имеет действующее удостоверение для задач, связанных с пользовательским интерфейсом, и задач файлов. Это удостоверение используется для определения применяемых политик (если таковые имеются). Если в качестве удостоверения задано значение "no identity" (без удостоверения) либо пользователь не является управляемым, никакие политики не применяются. На рисунках ниже показано, как определяются действующие удостоверения.
  ...
  ![Пакет SDK для приложений Intune для iOS — связанные платформы и библиотеки](../media/intune-app-sdk/ios-thread-identities.png)
...
## <a name="test-app-protection-policy-settings-in-xcode"></a>Проверка параметров политики защиты приложений в Xcode

Перед тестированием приложения с поддержкой Intune в рабочей среде вручную вы можете воспользоваться файлом Settings.bundle при работе в Xcode. Это позволит определить политики защиты приложений для тестирования без подключения к Intune.

### <a name="enable-policy-testing"></a>Включение тестирования политик

Выполните следующие действия, чтобы включить тестирование политик в Xcode.

1. Убедитесь, что используется отладочная сборка. Добавьте файл Settings.bundle, щелкнув правой кнопкой мыши папку верхнего уровня в проекте. Выберите **Add** (Добавить) > **New File** (Новый файл) в меню. Выберите шаблон **Settings Bundle** (Пакет настроек) в разделе **Resources** (Ресурсы).

2.  Скопируйте следующий блок в файл Settings.bundle/**Root.plist** для отладочной сборки:
    ```xml
...
3. В словаре **IntuneMAMSettings** в файле Info.plist приложения добавьте логическое значение "DebugSettingsEnabled". Для DebugSettingsEnabled следует задать значение "YES" (Да).



### <a name="app-protection-policy-settings"></a>Параметры политики защиты приложений

В следующей таблице описываются параметры политики защиты приложений, которые можно проверить с помощью файла MAMDebugSettings.plist. Чтобы включить параметр, добавьте его в MAMDebugSettings.plist.

| Имя параметра политики | Описание | Возможные значения |
| -- | -- | -- |
| AccessRecheckOfflineTimeout | Время в минутах, в течение которого приложение может выполняться в автономном режиме, прежде чем Intune заблокирует для приложения возможность запуска или возобновления работы, если включена проверка подлинности. | Любое целое число большее 0 |
|    AccessRecheckOnlineTimeout | Время в минутах, в течение которого приложение может выполняться, прежде чем пользователю потребуется ввести ПИН-код или выполнить проверку подлинности при запуске или возобновлении работы приложения (если для доступа включены проверка подлинности или использование ПИН-кода). | Любое целое число большее 0 |
| AppSharingFromLevel | Указывает, из каких приложений это приложение может принимать данные. | 0 = |
...
**Нужно ли регистрировать в службе APP-WE всех пользователей моего приложения?**
...
Этот метод должен вызываться перед выходом пользователя из приложения.  Если в рамках выхода данные пользователя удаляются из приложения, то для `doWipe` можно задать значение False. Но если приложение не удаляет данные пользователя, `doWipe` следует присвоить значение True, чтобы пакет SDK мог самостоятельно удалить эти данные.
