...
2. **Вариант 2**: выполните связывание `IntuneMAM.framework` с проектом. Перетащите библиотеку `IntuneMAM.framework` в список **Linked Frameworks and Libraries** (Связанные платформы и библиотеки) цели проекта.

    > [!NOTE]
    > При использовании платформы вам необходимо вручную убрать архитектуры симулятора из универсальной платформы перед отправкой приложения в App Store. Дополнительные сведения см. в статье [Руководство для разработчиков по пакету SDK для приложений Microsoft Intune в iOS](#Submit-your-app-to-the-App-Store).
...
7. Чтобы создать отдельную группу доступа, в которой будут храниться данные пакета SDK для приложений Intune, после включения совместного использования цепочки ключей сделайте следующее. Группу доступа к цепочке ключей можно создать при помощи пользовательского интерфейса или файла прав.
    ...
    2. Добавьте группу общей цепочки ключей `com.microsoft.intune.mam`. Пакет SDK для приложений Intune использует эту группу доступа для хранения данных.
...
10. Свяжите мобильное приложение с библиотекой проверки подлинности Azure Directory для iOS. Библиотека ADAL для Objective C доступна [на сайте GitHub](https://github.com/AzureAD/azure-activedirectory-library-for-objc).

    > [!NOTE]
    > Мы рекомендуем связывать приложение с последней рабочей версией ADAL.

11. Включите пакет ресурсов `ADALiOSBundle.bundle` в проект, перетащив его в узел **Copy Bundle Resources** (Копирование ресурсов пакета) раздела **Build Phases** (Этапы сборки).

12. Используйте параметр компоновщика `-force_load PATH_TO_ADAL_LIBRARY` при связывании с библиотекой.
...
**Как использовать кэш токена ADAL совместно с другими приложениями, подписанными при помощи того же профиля подготовки?**
...
> [!NOTE]
> Это необходимо при использовании политики защиты приложений без регистрации устройств, чтобы позволить пакету SDK многократно задействовать токен обновления ADAL, полученный приложением.
