...
|Microsoft Passport|Поддержка интеграции с Microsoft Passport for Work, являющаяся альтернативным методом входа для Windows 10, который использует Active Directory или учетную запись Azure Active Directory для замены пароля, смарт-карты или виртуальной смарт-карты.|[Управление параметрами Microsoft Passport на устройствах с помощью Microsoft Intune](/intune/deploy-use/control-microsoft-passport-settings-on-devices-with-microsoft-intune)|
