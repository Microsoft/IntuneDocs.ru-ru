...
15. Откройте приложение **Keychain Access** еще раз. Найдите сертификат, выполнив поиск слова **"iPhone"** в верхней правой панели поиска в окне Keychain Access. Щелкните правой кнопкой мыши элемент, чтобы открыть меню, и выберите пункт **Get Info** (Получить сведения).

  ![Добавление сертификата цепочке ключей](../media/app-wrapper/iOS-signing-cert-8.png)

16. Появляется информационное сообщение. Прокрутите его вниз и найдите метку **Fingerprints** (Отпечатки пальцев). Скопируйте строку **SHA1**, используемую в качестве параметра -c для инструмента упаковки приложений.
