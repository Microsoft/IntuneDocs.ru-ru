...
Ниже приведены некоторые проблемы, возникающие при регистрации устройств, а также способы их устранения.
...
1.  Перейдите по адресу [http://aka.ms/accessaad](http://aka.ms/accessaad) или выберите **Администрирование** &gt; **Azure AD** в [https://portal.office.com](https://portal.office.com).

2.  Выполните вход с помощью идентификатора организации, используя ссылку в левой части страницы.

3.  Создайте подписку Azure, если у вас ее нет. Если вы уже оплатили подписку, кредитная карта или оплата при этом не потребуется (щелкните ссылку подписки **Register your free Azure Active Directory** (Зарегистрировать бесплатную подписку Azure Active Directory)).
...
7.  Щелкните **Устройства**.

8.  Удалите соответствующие устройства, например неиспользуемые или имеющие неверные определения.

> [!NOTE]

> Можно избежать предела регистрации устройств с помощью диспетчеров регистрации устройств, как описано в статье [Регистрация корпоративных устройств с помощью диспетчера регистрации устройств в Microsoft Intune](/intune/deploy-use/enroll-corporate-owned-devices-with-the-device-enrollment-manager-in-microsoft-intune).
>
...
**Проблема:** на устройстве iOS или Android появляется ошибка **Profile installation failed** (Сбой установки профиля).
...
3.  Для устройства iOS перейдите по адресу [https://portal.manage.microsoft.com](https://portal.manage.microsoft.com) и попробуйте установить профиль при появлении запроса.

4.  Убедитесь, что Safari для iOS и Chrome для Android выбраны в качестве браузера по умолчанию и включены файлы cookie.
...
3.  В сценарии, где вы используете System Center Configuration Manager с Intune, убедитесь, что пользователь имеет допустимый облачный идентификатор пользователя:
    ...
    4.  В верхней части окна щелкните элемент "Создать запрос" и выполните следующие запросы:

        -   Для просмотра всех пользователей:

        -   Для просмотра определенных пользователей используйте следующий запрос, где %testuser1% представляет имя_пользователя@домен.com для пользователя, которого вы хотите найти:

        После написания запроса нажмите кнопку **!Выполнить**.
...
## Мобильные устройства исчезают при использовании System Center Configuration Manager с Intune
...
1. Отключите iOS внутри соединителя Windows Intune. 
    1. Щелкните правой кнопкой мыши подписку Intune и выберите пункт "Свойства".
...
1. Удалите этот раздел реестра, если он существует: ** HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\OnlineManagement regkey** и все его подразделы.
...
**Проблема:** при добавлении второго проверенного домена для AD FS пользователи с суффиксом имени субъекта-пользователя (UPN) этого второго домена могут быть не в состоянии входить на порталы или регистрировать устройства. 
...
|0x80043008, 0x80CF3008|Не удалось запустить службу Microsoft Online Management Updates.|Обратитесь в службу поддержки Майкрософт, как описано в статье [Получение поддержки для Microsoft Intune](how-to-get-support-for-microsoft-intune.md).|
...
## Ошибки регистрации iOS
Список других ошибок регистрации iOS приведен в документации по устройствам для пользователей в разделе [Возникновение ошибок при попытке регистрации устройства в Intune](/intune/enduser/using-your-ios-or-mac-os-x-device-with-intune).
