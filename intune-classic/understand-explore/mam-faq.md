...
**Как работают числовой PIN-код и секретный код для приложения Intune?**
Сейчас MAM позволяет использовать PIN-код уровня приложения (iOS), который содержит буквы, цифры и специальные символы ("секретный код"). При использовании этого секретного кода для интеграции пакета SDK для приложения Intune для iOS требуются приложения (например, WXP, Outlook, Managed Browser, Yammer). Без этих приложений параметры секретного кода неправильно применяются к целевым приложениям. Так как в приложениях регулярно выполняется интеграция, поведение секретного кода и числового PIN-кода временно изменено для пользователя и требует важного уточнения. Ниже описано поведение в выпуске Intune за октябрь 2017 г.

Приложения, соответствующие следующим условиям, смогут совместно использовать один секретный код:
1. Один издатель приложений.
2. Секретный PIN-код, который задается через консоль. 
3. Поддержка пакета SDK с этой функцией (версии 7.1.12+). 

Приложения, соответствующие следующим условиям, смогут совместно использовать числовой PIN-код:
1. Один издатель приложений.
2. Числовой PIN-код, который задается через консоль. 
