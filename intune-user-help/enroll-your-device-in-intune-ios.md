...
4. Появится экран **Выбор типа устройства и регистрации** , в котором будет предложено ввести тип устройства.  
    * Если вы получили ваше устройство от вашей организации, коснитесь пункта **(организация)** . Затем перейдите к разделу [Защита всего устройства](###secure-entire-device) в этой статье, чтобы завершить установку.  
    * Выберите **я принимаю это устройство** , если вы используете личное устройство, которое вы использовали дома. Затем перейдите к следующему шагу.  

    Если вы не видите этот экран, перейдите к разделу [Защита всего устройства](enroll-your-device-in-intune-ios.md#secure-entire-device) для завершения установки.  
    
    ![Пример снимка экрана корпоративный портал, "выбрать устройство и тип регистрации", параметры типа устройства.](./media/ios-device-type-1909.PNG)  


5. Выберите способ защиты данных на устройстве после его регистрации.  
    * Коснитесь элемента **безопасное целое устройство** , чтобы защитить все приложения и данные на устройстве. Затем перейдите к разделу [Защита всего устройства](enroll-your-device-in-intune-ios.md#secure-entire-device) для завершения установки.
    * Выберите **безопасные приложения и данные, связанные** с работой, только для защиты приложений и данных, доступ к которым осуществляется с помощью рабочей учетной записи. Затем перейдите к разделу [безопасные рабочие приложения и данные](enroll-your-device-in-intune-ios.md#secure-work-related-apps-and-data).  

    ![Пример снимка экрана корпоративный портал, "выбрать устройство и тип регистрации", параметры типа регистрации.](./media/ios-enrollment-type-1909.PNG)  


### <a name="secure-entire-device"></a>Защита всего устройства  

1. На экране **Управление устройствами и конфиденциальность** ознакомьтесь со списком сведений об устройстве, которые может и не видите ваша организация. Нажмите кнопку **Продолжить**.  
...
2. Safari откроет веб-сайт корпоративного портала на вашем устройстве. Когда будет предложено скачать профиль конфигурации, нажмите кнопку **Разрешить**. Если у вас устройство под управлением:  
    ...
    * iOS 12,1 и более ранние версии: по завершении загрузки вы автоматически перейдете в приложение параметров. Перейдите к шагу 4.  
 
    Если вы случайно коснетесь кнопки **Пропустить**, обновите страницу. Вам будет предложено открыть приложение корпоративного портала. После этого коснитесь элемента **скачать еще раз**.
...
3. При появлении запроса на открытие корпоративный портал нажмите кнопку **Открыть**. Ознакомьтесь со сведениями на экране **Установка профиля управления** .  

4. Перейдите в приложение параметров и выберите **Регистрация в < название организации >** или **профиль скачан**.  

    ![Пример снимка экрана приложения "Параметры", "регистрация в организации".](./media/enroll-in-organization-ios-1909.PNG)  

   Если ни один из этих параметров не отображается, перейдите в раздел **общие** > **Профили & Управление устройствами**> **профиля управления**. Если вы по-прежнему не видите профиль управления, может потребоваться скачать его еще раз.  
...
### <a name="secure-work-related-apps-and-data"></a>Безопасные приложения и данные, связанные с работой  
1. Откроется экран **загрузка Microsoft Authenticator** (если у вас уже есть средство проверки подлинности, вы не увидите этот экран, поэтому перейдите к шагу 2).  
    1. Коснитесь элемента **скачать в App Store**.
    2. Когда откроется магазин приложений, установите приложение. 
    3. Вернитесь к корпоративный портал и нажмите **продолжить**.    
    
   После установки Microsoft Authenticator не нужно предпринимать никаких действий с приложением. Он должен быть только на вашем устройстве. 

   ![Пример снимка экрана корпоративный портал, "Скачать Microsoft Authenticator".](./media/download-ms-authenticator-1909.PNG)  

2. На экране **Управление устройствами и конфиденциальность** ознакомьтесь со списком сведений об устройстве, которые может и не видите ваша организация. Нажмите кнопку **Продолжить**.  
...
3. Safari откроет веб-сайт корпоративного портала на вашем устройстве. Когда будет предложено скачать профиль конфигурации, нажмите кнопку **Разрешить**. Если у вас устройство под управлением:  
    ...
    * iOS 12,1 и более ранние версии: по завершении загрузки вы автоматически перейдете в приложение параметров. Перейдите к шагу 5.  
...
4. При появлении запроса на открытие корпоративный портал нажмите кнопку **Открыть**. Ознакомьтесь со сведениями на экране **Установка профиля управления** . 

5. Перейдите в приложение параметров и выберите **Регистрация в < название организации >** или **профиль скачан**.  
...
6. На экране **Регистрация пользователя** выберите **зарегистрировать мое iPhone**.  

    ![Пример снимка экрана приложения параметров, пользовательского экрана регистрации, который выделит кнопку регистрация.](./media/user-enrollment-information-1909.PNG)  

7. Введите пароль устройства. Нажмите кнопку **Установить**.  

8. На экране **входа** введите пароль для управляемого идентификатора Apple ID. В большинстве случаев эти учетные данные будут теми же, которые используются для входа в рабочую или учебную учетную запись, если только ваша организация не предоставила другой набор учетных данных. 
9. Нажмите **войти**.  
10. После установки профиля появится сообщение об успешном выполнении на экране. Чтобы проверить, установлен ли профиль, перейдите в раздел **профили & Управление устройствами** параметры. Вы должны увидеть профиль в списке  **Управление мобильными устройствами**.  
...
12. Вы поймете, что установка завершена, когда все элементы в списке будут показывать зеленый флажок. Коснитесь элемента **Готово**.  
