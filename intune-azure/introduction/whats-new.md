...
Дополнительные сведения см. на странице [Device restrictions for Android for Work](/intune-azure/configure-devices/device-restrictions-for-afw) (Ограничения устройств Android for Work).
...
Теперь бизнес-приложения для [iOS](/intune-azure/manage-apps/ios-lob-app) (IPA-файлы) и [Android](/intune-azure/manage-apps/android-lob-app) (APK-файлы) можно назначать пользователям или устройствам.
...
- **Приложения на начальном экране** — определяет, какие приложения пользователи видят на [начальном экране устройства iOS](/intune-azure/configure-devices/home-screen-settings-for-ios). Эта политика изменяет макет экрана домашней страницы, но не развертывает указанные приложения, если они не установлены.

- **Подключения к устройствам AirPrint** — управляет [устройствами AirPrint](/intune-azure/configure-devices/air-print-settings-for-ios-and-macos) (сетевыми принтерами), к которым могут подключаться пользователи устройства iOS.

- **Подключения к устройствам AirPlay** — управляет [устройствами AirPlay](/intune-azure/configure-devices/airplay-settings-for-ios-devices) (например, Apple TV), к которым могут подключаться пользователи устройства iOS.

- **Настраиваемое сообщение на экране блокировки** — настраивает собственное сообщение, которое пользователи будут видеть на экране блокировки устройств iOS (оно будет заменять сообщение на экране блокировки по умолчанию). Дополнительные сведения см. в разделе [Доступные действия на устройствах](/intune-azure/manage-devices/what-is#available-device-actions)


### <a name="restrict-push-notifications-for-ios-apps----723767---"></a>Ограничение push-уведомлений для приложений iOS <!-- 723767 -->

В профиле ограничений устройства Intune теперь можно настроить следующие [параметры уведомлений](/intune-azure/configure-devices/app-notification-settings-for-ios) для устройств iOS:
...
В профиле ограниченного использования устройств Intune теперь можно настроить следующие [параметры домена](/intune-azure/configure-devices/device-restrictions-for-ios#domains):
...
Теперь можно [синхронизировать книги](/intune-azure/manage-apps/ios-vpp-apps), приобретаемые в магазине Apple VPP Store с помощью Intune, и назначить их пользователям.
...
Устройства под управлением Samsung KNOX Standard теперь поддерживаются для [управления несколькими пользователями](/intune-azure/enroll-devices/enroll-android-and-knox-standard-devices) с помощью Intune. Это означает, что конечные пользователи могут выполнять вход и выход на устройстве с помощью учетных данных Azure Active Directory, а устройством можно управлять централизованно, независимо от того, используется оно или нет.  Когда пользователь входит в систему, он получает доступ к приложениям, кроме того, к нему применяются соответствующие политики. После выхода пользователя из системы все данные приложения удаляются.
...
Мы добавили поддержку дополнительных [параметров ограниченного использования устройств Windows](/intune-azure/configure-devices/device-restrictions-for-windows-10), таких как дополнительная поддержка браузера Edge, настройка экрана блокировки устройств, настройка начального меню, задание обоев поиска "Windows: интересное" и параметры прокси-сервера.
...
Мы добавили поддержку [управления несколькими пользователями](/intune-azure/enroll-devices/enroll-windows-devices) для устройств под управлением Windows 10 Creators Update, присоединенных к домену Azure Active Directory. Это означает, что все обычные пользователи, входящие на устройство с помощью учетных данных Azure AD, получат доступ ко всем приложениям и политикам, которые назначены их пользовательским именам. Сейчас пользователи не могут использовать корпоративный портал для таких сценариев самообслуживания, как установка приложений.

### <a name="fresh-start-for-windows-10-pcs---1004830---"></a>Fresh Start для ПК Windows 10<!-- 1004830 -->

В этом выпуске доступно новое [действие устройства Fresh Start](/intune-azure/manage-devices/what-is#available-device-actions) для компьютеров под управлением Windows 10.  При выполнении этого действия удаляются все приложения, которые были установлены на компьютере, и компьютер автоматически обновляется до последней версии Windows. Эту функцию можно использовать для удаления предварительно установленных OEM-приложений, которые часто поставляются с новыми компьютерами. Кроме того, можно настроить сохранение данных пользователей при выполнении этого действия устройства.

### <a name="additional-windows-10-upgrade-paths----903672---"></a>Дополнительные варианты обновления Windows 10 <!-- 903672 -->

Теперь можно создать [политику обновления выпусков, чтобы обновить устройства](/intune-azure/configure-devices/how-to-configure-windows-10-edition-upgrade) до следующих выпусков Windows 10:
...
Новый [портал для устранения неполадок](/intune-azure/manage-users/help-desk) позволяет операторам службы технической поддержки и администраторам Intune просматривать пользователей и их устройства и выполнять задачи для решения технических проблем Intune.
