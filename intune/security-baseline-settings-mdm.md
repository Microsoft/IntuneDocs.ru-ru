...
description: Основные настройки безопасности Intune для управления Windows 10
author: brenduns
ms.author: brenduns
manager: dougeby
ms.date: 06/20/2019
...
Просмотр параметров базовых показателей безопасности MDM, поддерживаемый Microsoft Intune для устройств под управлением Windows 10 или более поздней версии. Значения по умолчанию для параметров в этих базовых показателей представляют рекомендуемую конфигурацию для соответствующих устройств, а также может не соответствовать базовые значения по умолчанию из других базовых конфигураций системы безопасности.  

Является самой последней базовой версии **MDM базовой конфигурации системы безопасности для обновления Spring 2019 (19 H 1)**  
...
- **Установки MSI блока приложения с повышенными привилегиями**  
  ...
  - *Если включить этот параметр политики*, привилегии были расширены, чтобы все программы. Обычно эти привилегии зарезервированы для программ, назначенных пользователю (предложенных на рабочем столе), назначенных f компьютера (автоматически), или становятся доступными в Установка и удаление программ панели управления. Этот параметр профиля позволяет пользователям устанавливать программы, требующие доступа к каталогам, что пользователь может нет разрешения на просмотр или изменение, включая каталоги на компьютерах с высоким уровнем ограничений.
  - *Если отключить или не настраивать этот параметр политики*, система применяет разрешения текущего пользователя при установке программ, которые системный администратор так. Примечание. Этот параметр есть как в папке "Конфигурация компьютера", так и в папке "Конфигурация пользователя". Чтобы применить этот параметр политики, его необходимо включить в обеих папках. Предупреждение: Опытным пользователям воспользоваться преимуществами разрешения эта политика, необходимых для изменения своих прав и получить постоянный доступ к ограниченным файлам и папкам. Обратите внимание, что версия конфигурации пользователя, этот параметр политики не гарантирует безопасность.  
...
- **Поддержка шифрования Internet Explorer**  
  Этот параметр политики позволяет отключить поддержку для уровня безопасности транспорта (TLS) 1.0, TLS 1.1, TLS 1.2, Secure Sockets Layer (SSL) 2.0 или SSL 3.0 в браузере. TLS и SSL — это протоколы, защиты связи между браузером и на целевом сервере. Когда браузер пытается получить Настройка защищенного обмена данными с целевым сервером, какой протокол и версию, используемую браузером и сервером согласования. Обозреватель и сервер пытаются сопоставить друг друга список поддерживаемых протоколов и версий и они выбирают наиболее предпочтительную совпадение. Если этот параметр включен, браузер согласовывает или не выполняет согласование туннель шифрования с помощью методов шифрования, выберите из раскрывающегося списка. Если отключить или не настраивать этот параметр политики, пользователь может выбрать какие шифрования поддерживает метод браузера.  
  [Дополнительные сведения](https://go.microsoft.com/fwlink/?linkid=2067057)

  **По умолчанию**: 2 элемента: TLS версии 1.1 и TLS версии 1.2  
  *Щелкните стрелку вниз для отображения параметров, которые можно выбрать для этого параметра.*
...
- **Сценарии зона Интернета Internet Explorer**  
  Этот параметр политики позволяет управлять выполнением сценариев пользователем. Если этот параметр политики включен, пользователь может выполнять сценарии. Если этот параметр политики отключен, пользователь не может выполнять сценарии. Если этот параметр политики не настроен, пользователь может включать и отключать сценарии.  
...
- **Запрос удаленной помощи**  
  Этот параметр политики позволяет включить или отключить функцию удаленной помощи Solicited (запрашивать) на этом компьютере. 
  - *Если включить этот параметр политики*, пользователи на этом компьютере позволяет специалисту по электронной почте или передачи файлов. Кроме того пользователи могут использовать программы обмена мгновенными сообщениями, чтобы разрешить подключения к этому компьютеру, и можно настроить дополнительные параметры удаленного помощника. 
  - *Если вы отключаете этот параметр политики*, пользователи на этом компьютере не позволяет специалисту по электронной почте или передачи файлов. Кроме того пользователям нельзя использовать программы обмена мгновенными сообщениями, чтобы разрешить подключения к этому компьютеру. 
  - *Если вы не настроите этот параметр политики*, пользователи могут включить или отключить функцию удаленной помощи Solicited (запрашивать) сами в свойствах системы на панели управления. Пользователи также могут настраивать параметры удаленного помощника. 

  Если этот параметр включен, у вас есть два способа разрешить вспомогательные функции предоставлять удаленную помощь: «Помощники могут только наблюдать компьютера» или «Помощники могут управлять компьютером». «Максимальное время билета» параметр политики задает ограничение на количество времени, приглашение удаленного помощника, созданные с помощью электронной почты или передачи файлов может оставаться открытым. «Выбрать метод для отправки по электронной почте приглашения» параметр указывает, какие standard по электронной почте, чтобы использовать для отправки приглашений удаленного помощника. В зависимости от программу электронной почты можно использовать стандарт Mailto (получатель приглашения подключается через Интернет) или API управления СЛУЖБАМИ (простой MAPI) стандартный (приглашение присоединен к сообщению электронной почты). Этот параметр политики не в Windows Vista, так как API управления СЛУЖБАМИ — это единственный метод поддерживается. Если включить этот параметр политики также следует включить соответствующие исключения брандмауэра для разрешения подключения удаленного помощника.  
  [Дополнительные сведения](https://go.microsoft.com/fwlink/?linkid=2067198)

  **По умолчанию**: отключение удаленного помощника

  Если задано значение *включить удаленный помощник*, настройте следующие дополнительные параметры:  
  - **Разрешение запроса удаленной помощи**  
    **Значение по умолчанию:** "Просмотр"  

  - **Значение времени максимальное билета**  
    **Значение по умолчанию:** *Не настроено*  

  - **Билет максимальный период времени**  
    **По умолчанию**: минут    

  - **Метод приглашение по электронной почте**  
...
- **Контроль поведения**  
  Разрешает или запрещает функции наблюдения за поведением Защитника Windows. Эти встроенные в Windows 10 датчики собирают и обрабатывают сигналы о действиях, поступающие из операционной системы, а затем отправляют эти данные в частный изолированный облачный экземпляр ATP в Microsoft Defender.  
...
- **Require enhanced anti-spoofing, when available** (Требовать расширенную защиту от спуфинга, когда это возможно)  
  Если Да, устройства будут использовать расширенную защиту от спуфинга, если они доступны. Если нет, защиту от спуфинга будет заблокирована. Не настроен, будут учитываться настройки на клиенте.  
...
- **Требовать строчные буквы в PIN-коде**  
  При необходимости, ПИН-код пользователя должен включать по крайней мере одну строчную букву.

  **Значение по умолчанию**: "Разрешено"

- **Require special characters in PIN** (Требовать специальные символы в PIN-коде)  
  При необходимости, ПИН-код пользователя должен включать по крайней мере один специальный символ.

  **Значение по умолчанию**: "Разрешено"

- **Минимальная длина PIN-кода**  
  Минимальная длина ПИН-кода должна составлять от 4 до 127 символов.

  **По умолчанию:** 6

- **Требовать прописные буквы в PIN-коде**  
  При необходимости, ПИН-код пользователя должен включать по крайней мере одну прописную букву.
...
## <a name="whats-changed-in-the-new-template"></a>Изменения в новый шаблон
*MDM базовой конфигурации системы безопасности для обновления Spring 2019 (19H 1)* шаблон имеет следующие отличия от *предварительной версии* шаблона.

### <a name="changes-to-the-baseline-settings"></a>Изменения базовых показателей конфигурации
Доступны следующие параметры:
- *Новый* в последняя версия этого базового плана.
- *Удалить* из последней базовой версии, но была представлена в предыдущей версии.
...
- Бит политики съемных носителей Locker > **метод шифрования**
- **Корзина бит фиксированной политики диска** *(все параметры)*
- **Бит политику дисков системы locker** *(все параметры)*

*[Новое]* [**Возможность подключения**](#connectivity):
- **Настройте безопасный доступ к UNC-пути**

*[Новое]* [**Защита устройств**](#device-guard):
- **Virtualization based security** (Безопасность на основе виртуализации)


*[Новое]*  [ **DMA Guard**](#dma-guard):
- **Перечисление внешних устройств, несовместимых с защитой прямого доступа к памяти ядра**  
...
- **Internet Explorer internet зоны автоматический запрос для загрузки файлов** > значение по умолчанию — теперь **отключено**. В предварительной версии это была значение Enabled.
