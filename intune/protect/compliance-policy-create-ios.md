...
- **Требовать уровень угрозы на устройстве не больше указанного** *(iOS 8.0 и более новые версии)* .  
  Используйте этот параметр, чтобы принять оценку рисков в качестве условия соответствия. Выберите допустимый уровень угрозы:  
  - **Не настроено** (*по умолчанию*) — этот параметр не учитывается при оценке соответствия требованиям.
...
- **Минимальная требуемая ОС** *(iOS 8,0 и более поздние версии)* :  
...
- **Максимальная разрешенная версия ОС** *(iOS 8,0 и более поздние версии)* :  
...
- **Требовать пароль для разблокировки мобильных устройств**  
  - **Не настроено** (*по умолчанию*) — этот параметр не учитывается при оценке соответствия требованиям.  
  - **Требовать** — пользователи должны ввести пароль, прежде чем они смогут получить доступ к своему устройству. Устройства iOS, использующие пароли, шифруются.
...
- **Максимальный срок после блокировки экрана (в минутах), по истечении которого запрашивается пароль** *(iOS 8.0 и более поздние версии)* .  
  Укажите, как вскоре будет блокироваться экран, прежде чем пользователь должен ввести пароль для доступа к устройству. Параметры включают значение по умолчанию *не настроено*, *немедленно*и от *1 минуты* до *4 часов*.

- **Максимальное время бездействия (в минутах), по истечении которого экран блокируется**.  
  Введите время бездействия до блокировки экрана устройством. Параметры включают значение по умолчанию *не настроено*, *немедленно*и от *1 минуты* до *15 минут*.
...
- **Ограниченные приложения**:  
  ...
  - **Идентификатор пакета приложений** — введите уникальный идентификатор пакета, назначенный поставщиком приложения. Чтобы найти идентификатор пакета, см. раздел [Как найти идентификатор пакета приложения для iOS](https://support.microsoft.com/help/4294074/how-to-find-the-bundle-id-for-an-ios-app) (открывается другой сайт Майкрософт).  
