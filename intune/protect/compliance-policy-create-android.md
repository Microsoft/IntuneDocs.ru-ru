...
[Создание политики соответствия](create-compliance-policy.md#create-the-policy) В качестве **платформы**выберите **администратор устройств Android**.
...
- **Require the device to be at or under the Device Threat Level** (Требовать тот же или более низкий уровень риска устройства).  
  Используйте этот параметр, чтобы получить оценку рисков с подключенной мобильной службы защиты от угроз в качестве условия соответствия. 
  - **Не настроено** (*по умолчанию*) — этот параметр не учитывается при оценке соответствия требованиям. 
...
- **Шифрование хранилища данных на устройстве**.  
  *Поддерживается в Android 4,0 и более поздних версий или KNOX 4,0 и более поздних версиях.*  
  <br>
  - **Не настроено** (*по умолчанию*) — этот параметр не учитывается при оценке соответствия требованиям. Выберите 
  - **Обязательное** — шифрование хранилища данных на устройствах. Шифрование для устройств будет включено, если выбран параметр **Require a password to unlock mobile devices** (Запрашивать пароль для разблокировки мобильных устройств).  
