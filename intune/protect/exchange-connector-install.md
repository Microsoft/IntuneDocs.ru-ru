...
Перед внесением этого изменения убедитесь, что учетная запись, используемая для запуска соединителя Exchange, не используется для других целей управления Exchange. Это обусловлено тем, что Exchange имеет ограниченное число экземпляров пространства выполнения на одну учетную запись, большинство из которых будет использоваться соединителем. 
