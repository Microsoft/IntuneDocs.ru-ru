...
- Где находится сообщение об ошибке?
...
| **Не удается зарегистрировать устройства**  | Следующие причины могут быть применимы. <br> **-** [ ***Причина 1*** . приложение Jamf Pro в Azure имеет неверные разрешения](#cause-1) <br> **-** [ ***Причина 2*** — существует ошибка *соединителя Jamf Native macOS* в Azure AD.](#cause-2) <br> **-** [ ***Причина 3*** — у пользователя нет допустимой лицензии Intune или Jamf.](#cause-3) <br> **-** [ ***Причина 4*** . пользователь не использовал Jamf Self Service для запуска корпоративный портал приложения](#cause-4) <br> **-** [ ***Причина 5*** . Интеграция Intune отключена](#cause-5) <br> **-** [ ***Причина 6*** -устройство ранее было зарегистрировано в Intune или пользователь попытался зарегистрировать устройство несколько раз.](#cause-6) <br> **-** [ ***Причина 7*** -жамфаад запрашивает доступ к разделу "Microsoft Workplace Join Key" из цепочки пользователей.](#cause-7) |
...
В приложении корпоративный портал пользователь может увидеть **`Not registered`** , и в журналах корпоративный портал может появиться запись, аналогичная приведенной ниже.  
...
6. Удалите следующие файлы на устройстве, если они существуют:
   ...
   - /Users/<username>/Library/cookies/com. Microsoft. CompanyPortal. бинарикукиес
   - /Users/<username>/Library/cookies/com. jamf. Management. Жамфаад. бинарикукиес
...
**Причина**: интеграция Jamf с Intune не поддерживает политику соответствия, нацеленную на группы устройств. 
...
- Не выбран параметр **предоставления согласия администратора для *\<your компании >***  .  
