...
description: Используйте сертификаты PKCS с Microsoft Intune. Сюда входит работа с корневыми сертификатами и шаблонами сертификатов, установка соединителя сертификатов Intune (NDES) и профили конфигурации устройств для сертификата PKCS.
keywords: ''
author: brenduns
ms.author: brenduns
manager: dougeby
...
- **Соединитель сертификатов PFX для Microsoft Intune**.  
  ...
  См. сведения о всех сетевых конечных точках и доступе соединителя в описании [конечных точек сети для Microsoft Intune](../fundamentals/intune-endpoints.md).
...
3. Перейдите в раздел **Параметры** и настройте свойства, которые применяются к выбранной платформе:  
   
   |Параметр     | Платформа     | Подробные сведения   |
   |------------|------------|------------|
   |**Порог обновления (%)** .        |all         |Рекомендуется: 20 %.  | 
   |**Срок действия сертификата**.  |all         |Если вы не изменяли шаблон сертификата, для этого параметра следует задать один год. |
   |**Поставщик хранилища ключей (KSP)** .   |Windows 10  | Для Windows выберите место для хранения ключей на устройстве. |
   |**Центр сертификации**.      |all         |Отображает внутреннее полное доменное имя (FQDN) ЦС предприятия.  |
   |**Имя центра сертификации**. |all         |Отображает имя ЦС предприятия, например "Contoso Certification Authority". |
   |**Тип сертификата**.             |macOS       |Выберите тип. <br> **-** Тип сертификата **Пользователь** может содержать в субъекте или SAN атрибуты как пользователя, так и устройства. <br><br>**-** Тип сертификата **Устройство** может содержать в субъекте или SAN только атрибуты устройства. Используйте "Устройство" для таких сценариев, как устройства без пользователей, например киоски, или для других устройств общего пользования.  <br><br> Этот выбор влияет на формат имени субъекта. |
   |**Формат имени субъекта**.          |all         |На большинстве платформ следует задать для этого параметра значение **Обычное имя**, если не требуется иное.<br><br>Для macOS формат имени субъекта определяется типом сертификата. См. раздел [Формат имени субъекта для macOS](#subject-name-format-for-macos) далее в этой статье. |
   |**Альтернативное имя субъекта**.     |all         |Задайте для этого параметра значение **Имя участника-пользователя** (если не требуется иное). |
   |**Расширенное использование ключа**.           |**-** Администратор устройства с Android <br>**-** Android Enterprise (*владелец устройства*, *рабочий профиль*) <br> **-** Windows 10 |Для сертификата обычно требуется *Проверка подлинности клиента*, чтобы пользователь или устройство могли выполнить проверку подлинности на сервере. |
   |**Разрешить всем приложениям доступ к закрытому ключу**. |macOS  |Задайте значение **Разрешить**, чтобы предоставлять приложениям, настроенным для соответствующего устройства Mac, доступ к закрытому ключу сертификатов PKCS. <br><br> Дополнительные сведения об этом параметре см. в описании параметра *AllowAllAppsAccess* в разделе "Полезные данные сертификата" в [справочнике по профилю конфигурации](https://developer.apple.com/business/documentation/Configuration-Profile-Reference.pdf) из состава документации разработчика Apple. |
   |**Корневой сертификат**.             |**-** Администратор устройства с Android <br> **-** Android Enterprise (*владелец устройства*, *рабочий профиль*) |Выберите ранее назначенный профиль сертификата корневого ЦС. |
...
### <a name="subject-name-format-for-macos"></a>Формат имени субъекта для macOS

При создании профиля сертификата PKCS для macOS параметры для формата имени субъекта зависят от выбранного типа сертификата: **Пользователь** или **Устройство**.  
