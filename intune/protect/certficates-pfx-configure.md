...
4. Выберите **Параметры** и настройте свойства, которые применяются к выбранной платформе:
   ...
   |**Поставщик хранилища ключей (KSP)** .   |<ul><li>Windows 10  | Для Windows выберите место для хранения ключей на устройстве. |
   ...
   |**Формат имени субъекта**.          |<ul><li>Все         |На большинстве платформ следует задать для этого параметра значение **Обычное имя**, если не требуется иное.<br><br>Для указанных ниже платформ формат имени субъекта определяется типом сертификата. <ul><li>Android для бизнеса (*рабочий профиль*);</li><li>iOS</li><li>MacOS</li><li>Windows 10 и более поздней версии</li></ul>  <p> Ознакомьтесь с разделом [Формат имени субъекта](#subject-name-format) далее в этой статье. |
   |**Альтернативное имя субъекта**.     |<ul><li>Все         |Задайте для этого параметра значение **Имя участника-пользователя** (если не требуется иное). |
   |**Расширенное использование ключа**.           |<ul><li> Администратор устройства с Android </li><li>Android для бизнеса (*владелец устройства*, *рабочий профиль*); </li><li>Windows 10 |Для сертификата обычно требуется *Проверка подлинности клиента*, чтобы пользователь или устройство могли выполнить проверку подлинности на сервере. |
