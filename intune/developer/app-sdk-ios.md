...
1. **Проверьте WebView** . рекомендуется, чтобы приложения не использовали Сфсафаривиевконтроллер, Сфауссессион или асвебауссессион в качестве WebView для любых операций, инициированных приложением MSAL интерактивной аутентификации. Если по какой-либо причине приложение должно использовать одно из этих веб-представлений для любых интерактивных операций MSAL AUTH, оно также должно установить `SafariViewControllerBlockedOverride` в `true` в словаре `IntuneMAMSettings` в файле info. plist приложения. ПРЕДУПРЕЖДЕНИЕ. это приведет к отключению перехватчиков Сафаривиевконтроллер Intune для включения сеанса проверки подлинности. Это может привести к утечке данных в других местах приложения, если приложение использует Сафаривиевконтроллер для просмотра корпоративных данных, поэтому приложение не должно отображать корпоративные данные в любом из этих типов WebView.
2. **Связывание ADAL и MSAL** — разработчики должны принять участие в том случае, если требуется, чтобы INTUNE предпочитает MSAL через ADAL в этом сценарии. По умолчанию Intune будет предпочитать Поддерживаемые версии ADAL поддерживаемым версиям MSAL, если обе они связаны во время выполнения. Intune предпочитает использовать поддерживаемую версию MSAL, только если во время первой операции проверки подлинности Intune `IntuneMAMUseMSALOnNextLaunch` — `true` в `NSUserDefaults`. Если `IntuneMAMUseMSALOnNextLaunch` имеет значение `false` или не задано, Intune будет возвращаться к поведению по умолчанию. Как видно из названия, изменение `IntuneMAMUseMSALOnNextLaunch` вступит в силу при следующем запуске.
...
Пакет SDK Intune для iOS 9.0.3 + поддерживает возможность добавления консоли диагностики в мобильное приложение для тестирования политик и ведения журнала ошибок. `IntuneMAMDiagnosticConsole.h` определяет интерфейс класса `IntuneMAMDiagnosticConsole`, который разработчики могут использовать для вывода консоли диагностики Intune. Это позволяет конечным пользователям или разработчикам во время тестирования получать и совместно использовать журналы Intune для диагностики любых проблем, которые они могут иметь. Этот API является необязательным для интеграторов.
