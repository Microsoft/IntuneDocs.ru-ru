...
1. **Вариант 1 — платформа (рекомендуется)** . Если вы используете Xcode 10.2 + и ваше приложение или расширение содержит код SWIFT, свяжите `IntuneMAMSwift.framework` и `IntuneMAMSwiftStub.framework` с целевым объектом: перетащите `IntuneMAMSwift.framework` и `IntuneMAMSwiftStub.framework` в список **внедренных двоичных файлов** для целевого объекта проекта.

    В противном случае свяжите `IntuneMAM.framework` с целевым объектом: перетащите `IntuneMAM.framework` в список **внедренных двоичных файлов** для целевого объекта проекта.
...
MSAL — разработчики должны создать регистрацию приложения в AAD с помощью пользовательского URI перенаправления в указанном [здесь](https://github.com/AzureAD/microsoft-authentication-library-for-objc/wiki/Migrating-from-ADAL-Objective-C-to-MSAL-Objective-C#app-registration-migration)формате. Разработчикам следует задать ранее упомянутые параметры `ADALClientID` и `ADALRedirectUri` или эквивалентные свойства `aadClientIdOverride` и `aadRedirectUriOverride` в экземпляре `IntuneMAMPolicyManager`. Разработчики также должны убедиться, что они следуют за шагом 4 в предыдущем разделе, чтобы предоставить доступ к службе защиты приложений Intune для регистрации приложений.

### <a name="special-considerations-when-using-msal"></a>Специальные рекомендации при использовании MSAL 

1. **Проверьте WebView** . рекомендуется, чтобы приложения не использовали Сфсафаривиевконтроллер, Сфауссессион или асвебауссессион в качестве WebView для любых операций, инициированных приложением MSAL интерактивной аутентификации. Если по некоторым причинам приложение должно использовать одно из этих веб – представлений для любых интерактивных операций MSAL AUTH, необходимо также задать `SafariViewControllerBlockedOverride` для `true` в словаре `IntuneMAMSettings` в файле info. plist приложения. ПРЕДУПРЕЖДЕНИЕ. это приведет к отключению перехватчиков Сафаривиевконтроллер Intune для включения сеанса проверки подлинности. Это может привести к утечке данных в других местах приложения, если приложение использует Сафаривиевконтроллер для просмотра корпоративных данных, поэтому приложение не должно отображать корпоративные данные в любом из этих типов WebView.
2. **Связывание ADAL и MSAL** — разработчики должны принять участие в том случае, если требуется, чтобы INTUNE предпочитает MSAL через ADAL в этом сценарии. По умолчанию Intune будет предпочитать Поддерживаемые версии ADAL поддерживаемым версиям MSAL, если обе они связаны во время выполнения. Intune будет предпочитать поддерживаемую версию MSAL, только если во время первой операции проверки подлинности Intune `IntuneMAMUseMSALOnNextLaunch` `true` в `NSUserDefaults`. Если параметр `IntuneMAMUseMSALOnNextLaunch` `false` или не задан, Intune будет возвращаться к поведению по умолчанию. Как видно из названия, изменение `IntuneMAMUseMSALOnNextLaunch` вступит в силу при следующем запуске.
...
BackgroundColor| Строка| Задает цвет фона для компонентов пользовательского интерфейса пакета SDK Intune. Принимает шестнадцатеричную строку RGB в виде "#XXXXXX", где "X" может принимать значение от 0 до 9 или от A до F. Символ решетки можно пропустить.   | Необязательный параметр. По умолчанию используется системный цвет фона, который может различаться в разных версиях iOS и в соответствии с параметром темного режима iOS. |
ForegroundColor| Строка| Задает цвет переднего плана для компонентов пользовательского интерфейса пакета SDK Intune, например цвет текста. Принимает шестнадцатеричную строку RGB в виде "#XXXXXX", где "X" может принимать значение от 0 до 9 или от A до F. Символ решетки можно пропустить.  | Необязательный параметр. По умолчанию используется цвет системной метки, который может различаться в разных версиях iOS и в соответствии с параметром темного режима iOS. |
AccentColor | Строка| Задает контрастный цвет для компонентов пользовательского интерфейса пакета SDK Intune, таких как цвет текста кнопки и цвет выделения в поле ПИН-кода. Принимает шестнадцатеричную строку RGB в виде "#XXXXXX", где "X" может принимать значение от 0 до 9 или от A до F. Символ решетки можно пропустить.| Необязательный параметр. По умолчанию используется системный синий цвет. |
суппортсдаркмоде| Логическое значение | Указывает, должна ли цветовая схема пользовательского интерфейса пакета SDK Intune наблюдать за параметром темного режима системы, если для BackgroundColor/ForegroundColor/Акцентколор не задано явное значение. | Необязательный параметр. По умолчанию — yes. |
...
Дополнительные сведения о создании целевой политики конфигурации приложений MAM в iOS см. в разделе, посвященном целевой конфигурации MAM приложения, статьи [Использование политик конфигурации приложений Microsoft Intune для iOS](../apps/app-configuration-policies-use-ios.md).
