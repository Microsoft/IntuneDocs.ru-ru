...
Если приложение отображает уведомления, перед отображением уведомления необходимо проверить политику ограничения уведомлений для пользователя, связанного с уведомлением. Чтобы определить, применяется ли политика, выполните следующий вызов.
...
Если ограничение составляет `BLOCKED`, приложение не должно отображать уведомления для пользователя, связанного с этой политикой. Если `BLOCK_ORG_DATA`, приложение должно отобразить измененное уведомление, которое не содержит данных Организации. Если `UNRESTRICTED`, все уведомления разрешены.

Если `getNotificationRestriction` не вызывается, пакет SDK MAM будет использовать для автоматического ограничения уведомлений для приложений с одним удостоверением. Если автоматическая блокировка включена и `BLOCK_ORG_DATA` установлен, то уведомление не будет отображаться вообще. Для более точного управления проверьте значение `getNotificationRestriction` и соответствующим образом измените уведомления о приложении.
...
* **WIPE_USER_DATA**: это уведомление отправляется в классе `MAMUserNotification`. При получении этого уведомления приложение *должно* удалить все данные, связанные с управляемым удостоверением (из `MAMUserNotification.getUserIdentity()`). Уведомление может возникнуть по различным причинам, в том числе когда приложение вызывает `unregisterAccountForMAM`, когда он инициирует очистку или если не удовлетворены политики условного доступа, требуемые администратором. Если приложение не регистрируется для этого уведомления, будет выполняться очистка по умолчанию. Поведение по умолчанию приведет к удалению всех файлов для приложения с одним удостоверением или всех файлов, помеченных как управляемое удостоверение для приложения с несколькими удостоверениями. Это уведомление никогда не будет отправляться в потоке пользовательского интерфейса.
...
* **REFRESH_APP_CONFIG**: это уведомление отправляется в классе `MAMUserNotification`. При получении этого уведомления все кэшированные данные конфигурации приложений должны быть признаны недействительными и обновлены. Нет никаких гарантий в отношении потока, по которому будет отправляться это уведомление.
...
| `WRONG_USER` | Только один пользователь устройства может зарегистрировать приложение с использованием службы MAM. Этот результат означает, что пользователь, для которого был доставлен этот результат (второй пользователь), нацелен на политику MAM, но другой пользователь уже зарегистрирован. Так как политика MAM не может быть применена для второго пользователя, приложение не должно разрешать доступ к данным этого пользователя (возможно, путем удаления пользователя из приложения), если до этого пользователя не будет выполнена повторная попытка регистрации. При доставке этого `WRONG_USER` в MAM будет предложено удалить существующую учетную запись. Если пользователь ответит на голосами подтверждающими, вы действительно сможете зарегистрировать второго пользователя чуть позже. Пока второй пользователь остается зарегистрированным, MAM будет периодически повторять регистрацию. |
| `UNENROLLMENT_SUCCEEDED` | Отмена регистрации выполнена.|
| `UNENROLLMENT_FAILED` | Отмена регистрации не удалась.  Дополнительные сведения см. в журналах устройства. Как правило, это не происходит, пока приложение передает допустимое имя участника-пользователя (null или пустое). Нет прямого и надежного исправления, которое может потребоваться приложению. Если это значение получено при отмене регистрации действительного имени участника-пользователя, сообщите об ошибке команде Intune MAM.|
...
> [!NOTE]
> **Важное изменение!**  <br>
> Метод `MAMServiceAuthenticationCallback.acquireToken()` приложения должен передавать *значение false* для нового флага `forceRefresh` в `acquireTokenSilentSync()`.
...
> [!NOTE] 
> Настройка конфигурации для доставки через MAM — мы не можем делиеверед в `offline` (если корпоративный портал не установлен).  Только Android Enterprise AppRestrictions будут доставляться через `MAMUserNotification` при пустом удостоверении в этом случае.
...
Значение, заданное в конфигурации приложения MAM, переопределит значение с тем же набором ключей в Android Enterprise config. 

Если администратор настраивает конфликтующие значения для одного и того же ключа (например, для разных наборов конфигурации приложений с одним и тем же ключом в нескольких группах, содержащих одного и того же пользователя), Intune не имеет возможности автоматически устранить этот конфликт и сделает все значения доступно для вашего приложения. 
...
или запросите значение, которое необходимо выбрать:
```java
...
Конфигурацию приложения также можно настроить с помощью API Graph. Дополнительные сведения см. в разделе [API Graph документация по целевой конфигурации MAM](https://docs.microsoft.com/graph/api/resources/intune-mam-targetedmanagedappconfiguration).
