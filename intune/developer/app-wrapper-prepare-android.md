...
- Иногда в приложении может достигаться максимальный размер исполняемого файла Dalvik (DEX) из-за классов пакета SDK для Intune MAM, которые добавляются во время упаковки (необязательно). Файлы DEX являются частью компиляции приложения Android. Инструмент упаковки для приложений Intune автоматически обрабатывает переполнение файла DEX во время упаковки для приложений с минимальным уровнем API 21 или выше (начиная с [v. 
          ](https://github.com/msintuneappsdk/intune-app-wrapping-tool-android/releases)1.0.2501.1 Для приложений с минимальным уровнем API < 21 рекомендуется увеличить минимальный уровень API с помощью флага `-UseMinAPILevelForNativeMultiDex` оболочки. Чтобы клиенты не могли увеличить минимальный уровень API приложения, доступны следующие обходные пути DEX для переполнения. В некоторых организациях для этого может потребоваться взаимодействие с теми, кто компилирует приложение (например, с группой по сборке приложений).
...
|**-UseMinAPILevelForNativeMultiDex**| Используемых Используйте этот флаг, чтобы увеличить минимальный уровень API для исходного приложения Android до 21. Этот флаг запрашивает подтверждение, так как будет ограничивать, кто может устанавливать это приложение. Пользователи могут пропустить диалоговое окно подтверждения, добавив параметр "-Confirm: $false" в команду PowerShell. Этот флаг должен быть использован только клиентами в приложениях с мин. API < 21, которые не удается успешно перенести из-за ошибок переполнения DEX. | |
