...
- Иногда в приложении может достигаться максимальный размер исполняемого файла Dalvik (DEX) из-за классов пакета SDK для Intune MAM, которые добавляются во время упаковки (необязательно). Файлы DEX являются частью компиляции приложения Android. Инструмент упаковки для приложений Intune автоматически обрабатывает переполнение файла DEX во время упаковки для приложений с минимальным уровнем API 21 или выше (начиная с [V. 1.0.2501.1 Для приложений с уровнем API min < 21 рекомендуется увеличить минимальный уровень API, используя флаг оболочки `-UseMinAPILevelForNativeMultiDex`. Чтобы клиенты не могли увеличить минимальный уровень API приложения, доступны следующие обходные пути DEX для переполнения. В некоторых организациях для этого может потребоваться взаимодействие с теми, кто компилирует приложение (например, с группой по сборке приложений).
