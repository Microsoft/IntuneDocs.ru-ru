...
Если один профиль назначается приложению и вступает в силу, а затем назначается второй профиль, первый профиль получит приоритет и будет применяться, а второй будет считаться конфликтующим. Но если оба профиля применяются одновременно, т. е. предыдущего профиля нет, они оба будут конфликтующими. Поэтому для всех конфликтующих параметров задаются наиболее строгие значения.
...
При удалении профиля или устройства из группы, которой он был назначен, профиль и параметры удаляются с устройства согласно следующим спискам.
