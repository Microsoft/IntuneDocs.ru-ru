...
5. Используйте значения по умолчанию для следующих URL-адресов:
    ...
    > [!IMPORTANT]
    > Если пользователь является членом группы, для которой включена как автоматическая регистрация MDM, так и функция MAM, и пытается подключить свое личное устройство к рабочему месту, используется только MAM. 
