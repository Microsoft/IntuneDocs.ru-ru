...
- **Пароль.** Пользователь **должен** ввести пароль для доступа к устройству. Если выбрать вариант **Не настроено** (по умолчанию), доступ к устройству без пароля будет разрешен.
  ...
  - **Число неудачных попыток входа перед очисткой устройства**. Введите число неудачных попыток проверки подлинности, после которых выполняется очистка устройства (от 1 до 11). `0` (ноль) позволяет отключить функциональную возможность очистки устройства.

    Этот параметр оказывает разное влияние на различные выпуски. Подробные сведения см. в разделе [CSP DeviceLock/MaxDevicePasswordFailedAttempts](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-devicelock#devicelock-maxdevicepasswordfailedattempts).
