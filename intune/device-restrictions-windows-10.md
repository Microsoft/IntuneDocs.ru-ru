...
- **Только приложения из магазина**: этот параметр определяет взаимодействие с пользователем, если пользователи устанавливают приложения из мест, отличных от Microsoft Store. Доступны следующие параметры:

  - **Не настроено** (по умолчанию): позволяет конечным пользователям устанавливать приложения из мест, отличных от Microsoft Store, включая приложения, определенные в других параметров политики.  
  - **В любом месте**: отключает рекомендации приложения и позволяет пользователям устанавливать приложения из любого места.  
  - **Store только**: заставляет конечным пользователям устанавливать только приложения из Microsoft Store.
  - **Рекомендации по**: при установке приложения из Интернета, доступное в Microsoft Store, пользователи увидят сообщение, рекомендующее они загружают его из хранилища.  
  - **Предпочитать Store**: предупреждает пользователей при установке приложения из мест, отличных от Microsoft Store.
...
- **Пароль.** Пользователь **должен** ввести пароль для доступа к устройству. Если выбрать вариант **Не настроено** (по умолчанию), доступ к устройству без пароля будет разрешен. Применяется к только локальные учетные записи. Пароли учетных записей домена оставаться настроенными по Active Directory (AD) и Azure AD.
...
- **Мониторинг в реальном времени**. Значение **Включить** разрешает в режиме реального времени проверять наличие вредоносных программ, шпионского ПО и другого нежелательного программного обеспечения. Когда выбрано значение **Не настроено** (по умолчанию), эта функция разрешена.
...
- **Интервал обновления сигнатур (в часах)** . Укажите интервал, с которым Защитник проверяет наличие новых файлов сигнатур (от 0 до 24). Доступны следующие параметры:

  - **Не настроено** (по умолчанию)
  - **Не проверять**. Защитник не будет проверять наличие новых файлов сигнатур.
...
- **Проверять архивные файлы**. Значение **Включить** запрещает Защитнику проверять архивные файлы, например ZIP- или CAB-файлы. Когда выбрано значение **Не настроено** (по умолчанию), проверка разрешена.
...
- **Время для выполнения ежедневной быстрой проверки**: выберите час для выполнения ежедневной быстрой проверки. **Не настроено** — не выполнять ежедневную проверку. Если вам нужны дополнительные настройки, следует настроить параметр **Тип выполняемой проверки системы**.
  ...
  > [!WARNING]
  > Этот параметр в Intune на портале Azure может отображаться в состоянии сбоя. Это ошибка в компоненте отчетов. После воспроизведения поведения и устранения неполадок группа разработчиков продукта Intune подтверждает, что состояние фактически успешно. Эта проблема будет исправлена в будущих версиях. Сейчас срок неизвестен. Все обновления этой функции находятся [в разработке для Microsoft Intune](in-development.md).
...
- **Тип выполняемой проверки системы**: запланировать проверку системы, включая уровень сканирования, день и время для выполнения проверки. Доступны следующие параметры:
  ...
  - Для выполнения ежедневной быстрой проверки и ежедневной полной проверки выберите **Время для выполнения ежедневной быстрой проверки**. Задайте **Тип выполняемой проверки системы** как полную проверку с указанием дня и времени.
...
- **Действия с обнаруженными угрозами от вредоносных программ.** Выберите действия, которые Защитник должен выполнять для каждого обнаруженного уровня угроз (низкий, средний, высокий и критический). Если это невозможно, Защитник Windows сам выберет наилучший вариант, чтобы убедиться, что угрозы устранены. Доступны следующие параметры:
  - **Очистить**
  - **Карантин**
  - **Remove**
  - **Разрешить**
