...
-   **Режим терминала** — определяет тип [режима киоска](https://docs.microsoft.com/windows/configuration/kiosk-shared-pc), поддерживаемый этой политикой. Возможны следующие значения.
      ...
      - **Киоск с несколькими приложениями** — профиль позволяет использовать устройство в качестве киоска с несколькими приложениями.

    Для киосков с одним приложением нужно задать следующие параметры:

      - **Учетная запись пользователя** — указывает локальную (по отношению к устройству) учетную запись пользователя или имя для входа учетной записи Azure AD, связанное с приложением киоска. Для учетных записей, присоединенных к доменам Azure AD, укажите учетную запись в виде `domain\\username@tenant.org`.
    ...
    - **Имя конфигурации киоска** — понятное имя, используемое для идентификации заданной конфигурации.

    - Одно или несколько **приложений киоска** со следующими параметрами:

        - **Тип приложения**, который указывает на тип приложения киоска.  Поддерживаемые значения.   
