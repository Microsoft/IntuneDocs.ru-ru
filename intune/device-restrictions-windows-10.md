...
- **Мониторинг в реальном времени.** Значение **Включить** запрещает проверку вредоносных программ, шпионского ПО и другого нежелательного программного обеспечения в реальном времени. Когда выбрано значение **Не настроено** (по умолчанию), эта функция разрешена.
...
- **Контроль поведения.** Значение **Включить** запрещает Защитнику отслеживать наличие определенных типичных подозрительных действий на устройствах. **Не настроено** (по умолчанию) разрешает контроль поведения в Защитнике Windows.
...
- **Система проверки сети (NIS)**. Система NIS помогает защитить устройства от сетевых эксплойтов. Она использует сигнатуры известных уязвимостей из Microsoft Endpoint Protection Center, чтобы обнаружить и заблокировать вредоносный трафик.
  ...
  Когда выбрано значение **Не настроено** (по умолчанию), эта функция отключается. Пользователи не могут блокировать подключение к известным уязвимостям. Если задано значение **включить**, защита сети и блокировка сети включены, и пользователи не могут отключить ее. Пользователям запрещено подключаться к известным уязвимостям.
...
- **Сканировать все скачивания**: **не настроено** (по умолчанию). защитник просматривает все файлы, скачанные из Интернета. Если задано значение **включить**, эта функция отключена. Таким образом, защитник не проверяет все скачанные файлы Интернета.
...
- **Сканировать скрипты, загруженные в веб-браузеры Майкрософт**. Значение **Не настроено** (по умолчанию) разрешает Защитнику проверять скрипты, используемые в Internet Explorer. Значение **Включить** предотвращает эту проверку.
...
- **Доступ конечных пользователей к Защитнику**. Значение **Блокировать** скрывает пользовательский интерфейс Защитника Windows от конечных пользователей. Все уведомления Защитника Windows также подавляются. Значение **Не настроено** (по умолчанию) разрешает доступ пользователей к интерфейсу в Защитнике Windows. Изменение этого параметра вступает в силу при следующей перезагрузке компьютера.
...
- **Проверять архивные файлы**: **включите** отключение защитника для сканирования архивных файлов, таких как ZIP-или CAB-файлы. Когда выбрано значение **Не настроено** (по умолчанию), проверка разрешена.
...
- **Сканировать входящие сообщения электронной почты**. Значение **Включить** разрешает Защитнику проверять электронные письма при их поступлении на устройство. Когда выбрано значение **Не настроено** (по умолчанию), эта проверка отключается.
...
- **Сканировать съемные диски при полной проверке**. Значение **Включить** предотвращает полную проверку съемных носителей. **Не настроено** (по умолчанию) позволяет Защитнику проверять съемные носители, такие как USB-накопители.
...
- **Сканировать сопоставленные сетевые диски при полной проверке**. Значение **Включить** разрешает Защитнику проверять файлы на сопоставленных сетевых дисках. Когда выбрано значение **Не настроено** (по умолчанию), полная проверка отключается. Если файлы на диске доступны только для чтения, Защитник не сможет удалить найденные на нем вредоносные программы.
...
- **Сканировать файлы, открываемые из сетевых папок**. Значение **Не настроено** (по умолчанию) разрешает Защитнику проверять файлы на общих сетевых дисках (например, доступные по UNC-пути). Значение **Включить** предотвращает эту проверку. Если файлы на диске доступны только для чтения, Защитник не сможет удалить найденные на нем вредоносные программы.
...
- **Защита облака**. Значение **Не настроено** (по умолчанию) разрешает Microsoft Active Protection Service получать с управляемых устройств сведения об активности вредоносных программ. Значение **Включить** блокирует эту функцию.
