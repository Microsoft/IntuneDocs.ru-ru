...
|APNSCertificateNotValid|Возникла проблема с сертификатом, который позволяет мобильному устройству взаимодействовать с корпоративной сетью.<br /><br />|Служба Apple Push Notification Service (APNs) предоставляет возможность доступа к зарегистрированным устройствам iOS. Если действия по получению сертификата APNs не были выполнены или истек срок его действия, попытки регистрации завершатся выводом этого сообщения об ошибке.<br /><br />Просмотрите сведения о настройке пользователей в статьях [Синхронизация Active Directory и добавление пользователей в Intune](users-add.md) и [Организация пользователей и устройств](groups-add.md).|
|AccountNotOnboarded|Возникла проблема с сертификатом, который позволяет мобильному устройству взаимодействовать с корпоративной сетью.<br /><br />|Служба Apple Push Notification Service (APNs) предоставляет возможность доступа к зарегистрированным устройствам iOS. Если действия по получению сертификата APNs не были выполнены или истек срок его действия, попытки регистрации завершатся выводом этого сообщения об ошибке.<br /><br />Дополнительные сведения см. в статье [Настройка управления устройствами iOS и Mac с помощью Microsoft Intune](ios-enroll.md).|
...
|UserLicenseTypeInvalid|Нельзя зарегистрировать устройство, так как ваша учетная запись еще не входит в нужную группу пользователей.<br /><br />|Чтобы пользователь мог зарегистрировать свое устройство, он должен быть участником соответствующей группы пользователей. Это сообщение означает, что пользователь располагает неправильным типом лицензии для назначенного центра управления мобильными устройствами. Например, если в качестве центра управления мобильными устройствами назначен Intune, а пользователь использует System Center 2012 R2 Configuration Manager, появится указанное сообщение об ошибке.<br /><br />Дополнительные сведения см. в следующих статьях:<br /><br />Изучите статью, посвященную [настройке управления устройствами c iOS и Mac OS с помощью Microsoft Intune](ios-enroll.md), а также сведения о настройке пользователей в статьях [Синхронизация Active Directory и добавление пользователей в Intune](users-add.md) и [Организация пользователей и устройств](groups-add.md).|
|MdmAuthorityNotDefined|Центр управления мобильными устройствами не определен.<br /><br />|В Intune не был назначен центр управления мобильными устройствами.<br /><br />См. пункт 1 в разделе "Шаг 6: регистрация мобильных устройств и установка приложения" статьи [Начало работы с 30-дневной пробной версии Microsoft Intune](free-trial-sign-up.md).|
...
Дополнительные сведения см. в статье [Рекомендации по защите служб федерации Active Directory](https://technet.microsoft.com/windows-server-docs/identity/ad-fs/operations/best-practices-securing-ad-fs). Если вам нужна помощь, чтобы определить, включена ли у вашего поставщика удостоверений федерации конечная точка WS-Trust 1.3 в режиме "Имя пользователя/смешанный", обратитесь в службу технической поддержки Майкрософт (в случае использования служб AD FS) или к стороннему поставщику удостоверений.
...
3.  Перейдите на сайт [https://portal.manage.microsoft.com](https://portal.manage.microsoft.com) и попытайтесь установить профиль при появлении запроса.
