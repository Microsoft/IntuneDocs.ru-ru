...
ms.locfileid: "39321735"
---
## <a name="wi-fi-settings-for-windows-10-and-later-devices-in-intune"></a>Параметры Wi-Fi для устройств Windows 10 и более поздних версий в Microsoft Intune

Параметры Wi-Fi используются в профиле конфигурации, который применяется к устройствам под управлением Windows 10 и более поздних версий. Доступные варианты:
...
## <a name="settings-for-basic-and-enterprise-profiles"></a>Параметры Wi-Fi для простых и корпоративных профилей
...
## <a name="settings-for-basic-profiles-only"></a>Параметры только для простых профилей
...
## <a name="settings-for-enterprise-profiles-only"></a>Параметры только для корпоративных профилей

- **Единый вход (SSO)**. Позволяет настроить единый вход (SSO), при котором одни учетные данные используются для входа как в сеть Wi-Fi, так и в компьютер. Доступны следующие параметра:
...
### <a name="more-options-when-you-choose-the-eap-type"></a>Дополнительные параметры при выборе типа EAP
...
| Имя параметра | Дополнительные сведения | Когда используется |
|---|---|---|
| **Сертификат клиента для проверки подлинности клиента (сертификат удостоверения)** |  Выберите профиль сертификата SCEP, используемый для проверки подлинности подключения. | Тип EAP — **EAP-TLS**. |
| **Метод проверки подлинности** | Выберите метод проверки подлинности для подключения:<br><br>- **Сертификаты**. Выберите клиентский сертификат SCEP, который является сертификатом удостоверения для сервера.<br><br>- **Имя пользователя и пароль**. Введите метод проверки подлинности **Метод, отличный от EAP (внутреннее удостоверение)**. Доступны следующие параметры:<br><br>- **Незашифрованный пароль (PAP)**<br>- **Протокол проверки пароля (CHAP)**<br>- **Microsoft CHAP (MS-CHAP)**<br>- **Microsoft CHAP версии 2 (MS-CHAP v2)**<br><br>- **Конфиденциальность удостоверения (внешнее удостоверение)**. Введите текст, отправляемый в ответ на запрос удостоверения EAP. Этот текст может иметь любое значение. Во время проверки подлинности сначала будет отправлено это анонимное удостоверение, после чего в защищенный туннель будет отправлена фактическая идентификация. | Тип EAP — **EAP-TTLS** |
