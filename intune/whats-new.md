...
Теперь в колонке **Устранение неисправностей** вы можете просмотреть ограничения регистрации, действующие для каждого пользователя, выбрав **Ограничения регистрации** из списка **Назначения**.
...
Для устройств с браузером Edge теперь доступны два новых параметра: **Путь к избранным файлам** и **Изменения в избранном**. 
...
Вам ничего не нужно делать, чтобы подготовиться к этому изменению, поскольку это незначительное обновление пользовательского интерфейса рабочего процесса. Дополнительные сведения о соответствии GDPR корпорации Майкрософт см. в центре управления безопасностью по ссылке в разделе "Дополнительные сведения".
...
#### <a name="what-do-i-need-to-do-to-prepare-for-this-change"></a>Как можно подготовиться к этому изменению?
Добавьте в избранное колонку Intune вместо колонки службы "Защита приложений Intune" и ознакомьтесь с рабочим процессом политики защиты приложений в колонке "Мобильное приложение" в Intune. В течение некоторого времени будет происходить перенаправление, после чего колонка "Защита приложений Intune" будет удалена. Помните, что все политики защиты приложений уже перенесены в Intune и вы можете изменять любые политики условного доступа, используя инструкции в следующей документации: [https://aka.ms/azuread_ca](https://aka.ms/azuread_ca).
...
Процедура прекращения регистрации для устройств с OS X Yosemite 10.10 и более ранними версиями macOS начнется в феврале 2018 г. Intune полностью поддерживает OS X El Capitan 10.11 и более поздних версий.

### <a name="new-path-for-managed-devices-in-graph-api----1586728---"></a>Новый путь к управляемым устройствам в API Graph <!-- 1586728 -->
Изменяется путь, использовавшийся для доступа к управляемым устройствам в бета-версии API Graph. 
