...
Теперь можно блокировать изменения даты и времени на устройствах Samsung Knox. Эту функцию можно найти в разделе **Профили конфигурации устройств** > **Ограничения для устройств (Android)** > **Общие**.
...
Теперь вы можете устанавливать приложения Office на устройствах macOS. Этот новый тип позволяет установить Word, Excel, PowerPoint, Outlook и OneNote. Эти приложения также поставляются с Microsoft AutoUpdater (MAU), чтобы обеспечить безопасность и обновление приложений.
...
Мы обновили несколько интерфейсов API Graph для Intune, которые доступны в бета-версии. Чтобы получить дополнительные сведения, ежемесячно просматривайте [журнал изменений в API Graph](https://developer.microsoft.com/graph/docs/concepts/changelog).
