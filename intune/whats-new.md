...
В Intune теперь поддерживается регистрация Windows Autopilot без проверки подлинности пользователя. Это новый параметр в профиле развертывания Windows Autopilot "Режим Autopilot Deployment" со значением "Самостоятельное развертывание".  Для использования этого типа регистрации устройство должно работать по управлением ОС Windows 10 сборки 1803 или более поздней и быть оснащено доверенным платформенным модулем (TPM) версии 2.0. Так как проверка подлинности пользователя не требуется, этот параметр следует задавать только для устройств, над которыми имеется физический контроль.
...
Чтобы просмотреть уникальный идентификатор устройства (UDID) для устройства iOS или macOS, выберите **Устройства** > **Все устройства** > выберите устройство > **Оборудование**. Идентификаторы UDID доступны только для корпоративных устройств (задается с помощью свойства **Устройства** > **Все устройства** > выберите устройство > **Свойства** > **Владение устройством**).
