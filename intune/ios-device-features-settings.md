...
ms.locfileid: "55072547"
---
# <a name="ios-device-feature-settings-in-intune"></a>Настройка параметров для функций устройств iOS в Intune
...
1. В меню **Параметры** выберите **AirPrint**. Введите следующие свойства сервера AirPrint:
    ...
    - **IP-адрес**: Введите IPv4- или IPv6-адрес принтера. Если для обозначения принтеров используются имена узлов, IP-адрес можно получить, проверив связь с принтером с помощью терминала. Дополнительные сведения см. в этой статье в разделе [Получение IP-адреса и пути](#get-the-ip-address-and-path).
    - **Путь**. Как правило, для принтеров в сети путь будет таким: `ipp/print`. Дополнительные сведения см. в этой статье в разделе [Получение IP-адреса и пути](#get-the-ip-address-and-path).
...
1. В меню **Параметры** выберите **Макет начального экрана (только в защищенном режиме)** > **Панель закрепления** > **Добавить**. На панели закрепления устройства вы можете добавить не более **шести** элементов (как приложений, так и папок).
2. В меню **Тип** добавьте **приложение** или **папку**.
    ...
    - **Добавить приложение**. Выберите этот параметр, чтобы добавить приложения на панель закрепления на экране. Введите:
      ...
      - **Идентификатор набора приложений**. Введите идентификатор пакета приложения. Примеры см. в перечне [идентификаторов пакета для встроенных приложений iOS](#bundle-ids-for-built-in-ios-apps).
...
![Пример панели закрепления для iOS на iPhone](./media/bAgCe8F.png)
...
2. Укажите следующие свойства.
    ...
    - **Идентификатор пакета приложений**. Укажите **идентификатор пакета приложений** для приложения, которое требуется добавить. Примеры см. в перечне [идентификаторов пакета для встроенных приложений iOS](#bundle-ids-for-built-in-ios-apps).
...
1. В меню **Параметры** выберите **Конфигурация общего устройства (только в защищенном режиме)**.
2. Введите следующие параметры:

    - **Сведения о теге ресурса**. Введите сведения о теге ресурса устройства. Например, введите `Owned by Contoso Corp`. 

      Вводимый текст отображается на экране входа и окна блокировки устройства.

    - **Примечание на экране блокировки**. Введите примечание, которое поможет вернуть устройство в случае его потери или кражи. Например, можно ввести `If found, call Contoso at ...`.
...
1. В меню **Параметры** выберите **Фильтр веб-содержимого (только в защищенном режиме)**.
