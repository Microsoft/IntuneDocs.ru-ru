...
После возвращения получателя приложения больше не будет иметь доступ к ключам шифрования.
...
`getComplianceStatus()` Метод возвращает результат попытки исправления соответствия в виде значения из `MAMCAComplianceStatus` перечисления.
...
Удостоверение, заданное на уровне потока, заменяет удостоверение, заданное на уровне `Context`, которое, в свою очередь, заменяет удостоверение, заданное на уровне процесса. Удостоверение, заданное в контексте `Context`, используется только в соответствующих связанных сценариях. Например, с операциями ввода-вывода файлов никакой контекст `Context` не связан. Чаще всего приложения задают `Context` удостоверения на `Activity`. Приложение *не должно* отображать данные для управляемого удостоверения, если оно не совпадает с удостоверением `Activity`. В общем случае удостоверение уровня процесса помогает, только если приложение в любой момент работает только с одним пользователем во всех потоках. Во многих приложениях это не требуется.

Если приложение использует `Application` контекст получения системных служб, убедитесь, что удостоверение потока или процесса, установлена, или необходимо установить удостоверение пользовательского интерфейса на ваше приложение `Application` контекста.

Для обработки особых случаях при обновлении удостоверение пользовательского интерфейса с помощью `setUIPolicyIdentity` или `switchMAMIdentity`, оба метода могут передаваться набор `IdentitySwitchOption` значения.

* `IGNORE_INTENT`: Используется, если запрашивает переключение удостоверения, которое следует игнорировать намерения, связанный с текущим действием.
  Пример.

  1. Приложение получает объект intent от управляемого удостоверения, содержащего управляемого документа, а приложение отображает документ.
  2. Пользователь перешел в личных учетных данных, поэтому приложение запрашивает замена удостоверений пользовательского интерфейса. В личной идентификации, приложение больше не отображает документ, поэтому использовать `IGNORE_INTENT` при запросе в переключении удостоверения.

  В противном случае набор, пакет SDK предполагает, что последние целью по-прежнему используется в приложении. В результате будут получать политики для нового удостоверения обрабатывать цель как входящие данные и использовать его идентификатор.

>[!NOTE]
> Так как `CLIPBOARD_SERVICE` используется для операций пользовательского интерфейса в пакете SDK используется удостоверение пользовательского интерфейса для переднего плана действий `ClipboardManager` операций.
...
Если не переопределить `onSwitchMAMIdentityComplete` (или вызвать `super` метод), приведет к коммутатору неудачных удостоверений в действии в действии завершено. При переопределении метода, необходимо соблюдать осторожность, что корпоративные данные не отображается после переключения неудачных удостоверений.
...
Необходимо использовать, если асинхронная операция написать корпоративные данные в файл или может взаимодействовать с другими приложениями.
...
#### <a name="single-identity-to-multi-identity-transition"></a>Единую идентификацию переход множественной идентификации
Если приложение, выпущенной ранее с одним удостоверением позже интеграцией Intune интегрируется с множественной идентификации, ранее установленные приложения будут возникать переход (не отображается для пользователя, он не связан UX). Приложение не *требуется* делать что-либо явной обработки такого перехода. Все файлы, созданные до перехода будут по-прежнему выполняется рассматривать как управляемый (поэтому они будут остаются зашифрованными, если включена политика шифрования). При желании можно обнаружить обновления и использовать `MAMFileProtectionManager.protect` пометить определенные файлы или каталоги с пустым удостоверением (что удалит шифрования, если они были зашифрованы).
...
Если вы не наследуете `MAMContentProvider` явным образом и вместо этого разрешить средства построения, чтобы внести это изменение, можно назвать статическая версия метода: `MAMContentProvider.isProvideContentAllowed(provider, contentIdentity)`.
...
Выборочной очистки по умолчанию будет корректно, закройте приложение, завершение действия и выполняется прерывание процесса приложения. Если приложение переопределяет выборочной очистки по умолчанию, может потребоваться рекомендуется закрыть приложения вручную, чтобы запретить пользователям доступ к данным в памяти, после очистки.
...
Пары ключ значение конкретного приложения, которые могут быть настроены в консоли Intune [MAM — МЫ](https://docs.microsoft.com/intune/app-configuration-policies-managed-app) и [приложениями с Android рабочим профилем](https://docs.microsoft.com/intune/app-configuration-policies-use-android).
...
> [!NOTE] 
> Настройка конфигурации для передачи с помощью MAM-МЫ не может доставить в `offline`.  Только Android Enterprise AppRestrictions будут предоставляться через `MAMUserNotification` на пустое удостоверение в данном случае.
...
> [!NOTE] 
> **По умолчанию регистрации** учитывает национальные облака.

Включите регистрацию по умолчанию, выполнив следующие действия:

1. Если приложение включает ADAL, или необходимо включить единый вход, [Настройка ADAL](#configure-azure-active-directory-authentication-library-adal) следующие [распространенные конфигурации ADAL](#common-adal-configurations) #2. В противном случае этот шаг можно пропустить.
...
Пакет SDK для приложений Intune для Android не управляет сбором данных из приложения. Приложение корпоративного портала записывает системные данные по умолчанию. Эти данные отправляются в Microsoft Intune. Согласно политике конфиденциальности Майкрософт Корпорация Майкрософт не собирает личные данные.
