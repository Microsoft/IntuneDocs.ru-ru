...
4. Если выше вы выбрали **Динамическое устройство** для **Типа членства**, то в колонке **Группа** выберите **Динамические члены устройства**, а в поле **Расширенное правило** введите какой-то из следующих вариантов кода.
    ...
    - Поле тега группы Intune сопоставляется с атрибутом OrderID на устройствах Azure AD. Если вам нужно создать группу со всеми устройствами Autopilot, имеющими определенный тег группы (OrderID), введите `(device.devicePhysicalIds -any _ -eq "[OrderID]:179887111881")`.
...
5. На странице **Готовый интерфейс (OOBE)** для параметра **Режим развертывания** выберите один из двух вариантов:
    ...
    - **Саморазвертывание (предварительная версия)** : устройства с этим профилем (требуется Windows 10 версии 1809 или более поздней версии) не связаны с пользователем, регистрирующим устройство. Для регистрации устройства не нужны учетные данные пользователя.
...
7. Настройте следующие параметры.
    ...
    >[!IMPORTANT]
    >Для развертываний Autopilot на устройствах Windows 10 версии 1903 и более поздних для параметра"Данные диагностики" по умолчанию задано значение "Полные". Дополнительные сведения см. в разделе [Данные диагностики Windows](https://docs.microsoft.com/windows/privacy/windows-diagnostic-data). <br>
...
> [!NOTE]
> Intune будет периодически проверять наличие новых устройств в назначенных группах и затем начнет процесс назначения профилей на этих устройствах. Выполнение может занять несколько минут. Перед развертыванием устройства убедитесь, что этот процесс завершен.  Вы можете проверить это в разделе **Регистрация устройств** > **Регистрация Windows** > **Устройства**, где состояние профиля должно измениться с "Не назначено" на "Назначается" и, наконец, на "Назначено".
...
- Удалите устройства из Windows Autopilot в разделе **Регистрация устройств** > **Регистрация Windows** > **Устройства**. Выберите удаляемые устройства и нажмите **Удалить**. Удаление устройств Windows Autopilot может занять несколько минут.
...
1. Если устройства зарегистрированы в Intune, сначала [удалите их на колонке "Все устройства" Intune](devices-wipe.md#delete-devices-from-the-azure-active-directory-portal).

2. Удалите устройства в разделе устройств Azure Active Directory: **Устройства** > **Устройства Azure AD**.

3. Удалите устройства из Windows Autopilot в разделе **Регистрация устройств** > **Регистрация Windows** > **Устройства**. Выберите удаляемые устройства и нажмите **Удалить**. Удаление устройств Windows Autopilot может занять несколько минут.
