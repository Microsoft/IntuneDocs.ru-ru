...
- **Включить управление доступом к сети (NAC)** (только Citrix SSO). При выборе **I agree** (Соглашаюсь) идентификатор устройства добавляется в профиль VPN. Этот идентификатор может использоваться для аутентификация в VPN для разрешения или блокирования доступа к сети.
