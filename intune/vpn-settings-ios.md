...
- **Включить управление доступом к сети (NAC)**: этот параметр является заполнителем для клиентов VPN, таких как Citrix, чтобы разрешить идентификатор устройства в профиле VPN для использования с управлением доступом к сети (NAC). Когда вы нажимаете **Принимаю**, идентификатор устройства добавляется в профиль VPN. В настоящее время не существует партнерских решений для клиентов VPN или NAC, которые поддерживают этот новый идентификатор, поэтому устройствам будет разрешено подключаться к VPN независимо от состояния соответствия требованиям. Мы обновим этот документ, когда наши партнеры добавят поддержку идентификатора.

  Важные сведения.  

  - Если этот параметр включен, VPN отключается каждые 24 часа.
  - Идентификатор устройства является частью профиля, но он не отображается в Intune или в профиле. Майкрософт не сохраняет и не передает этот идентификатор. Когда партнеры VPN будут поддерживать этот идентификатор, VPN-клиент, например единый вход Citrix, сможет получить его и запросить у Intune подтверждение регистрации устройства и состояние соответствия профиля VPN.
