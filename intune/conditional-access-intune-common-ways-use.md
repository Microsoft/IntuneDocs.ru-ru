...
Intune оценивает состояние устройства и управляет им.
...
Служба Intune в партнерстве с поставщиками служб защиты мобильных устройств от угроз (Mobile Threat Defense) предлагает решение для обеспечения безопасности, которое позволяет обнаруживать вредоносные программы, трояны и другие угрозы для мобильных устройств.
...
Условный доступ для ПК позволяет реализовать возможности, аналогичные доступным для мобильных устройств. Рассмотрим способы использования условного доступа при управлении ПК с помощью Intune.

#### <a name="corporate-owned"></a>Корпоративные устройства

-   **Локальные устройства, присоединенные к домену AD**. Это наиболее распространенный для организаций вариант развертывания среды условного доступа. Он применяется в тех случаях, когда организации удовлетворены уровнем защиты, который обеспечивает им управление ПК с использованием групповых политик AD и (или) решения System Center Configuration Manager.

-   **Устройства, присоединенные к домену Azure AD и находящиеся под управлением Intune**. Этот сценарий, как правило, применяется в тех случаях, когда пользователи самостоятельно выбирают устройство или используют мобильные ноутбуки, редко подключающиеся к корпоративной сети. Устройство присоединяется к Azure AD и регистрируется в Intune, в результате чего удаляются любые зависимости в локальной среде AD и на контроллерах домена. Это может использоваться в качестве критерия условного доступа при доступе к корпоративным ресурсам.
