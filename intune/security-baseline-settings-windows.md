...
- **Bit locker removable drive policy** (Политика BitLocker в отношении съемных носителей)  
  ...
  Для политики съемных носителей locker бит настройте следующие параметры:

    - **Require encryption for write access** (Требовать шифрование для доступа на запись)  
      **По умолчанию**: Да  
  
    - **Метод шифрования**  
      **По умолчанию**: AES 256-разрядный CBC  

- **Bit locker fixed drive policy** (Политика BitLocker в отношении жестких дисков)  
  Этот параметр политики используется для определения метода шифрования и стойкости шифра. Значения этой политики определяют стойкость шифра, используемого BitLocker для шифрования. Предприятиям может потребоваться управлять уровнем шифрования для повышения уровня безопасности (AES-256 надежнее, чем AES-128). Если этот параметр включен, можно настраивать алгоритм шифрования и стойкость шифра ключа для жестких дисков с данными, дисков операционной системы и съемных носителей с данными по отдельности. Для жестких дисков и дисков операционной системы рекомендуется использовать алгоритм XTS-AES. Для съемных носителей следует использовать алгоритм "AES-CBC, 128 бит" или "AES-CBC, 256 бит", если носитель будет использоваться на других устройствах, работающих не под управлением Windows 10 версии 1511 или более поздней. Изменение метода шифрования не оказывает никакого влияния, если диск уже зашифрован или находится в процессе шифрования. В этом случае данный параметр политики игнорируется.  
 
   Для Bit locker фиксированной политики диска настройте следующие параметры: 
   - **Метод шифрования**
     **по умолчанию**: AES 256-разрядный XTS  

- **Bit locker system drive policy** (Политика BitLocker в отношении системных дисков)  
  Этот параметр политики используется для определения метода шифрования и стойкости шифра. Значения этой политики определяют стойкость шифра, используемого BitLocker для шифрования. Предприятиям может потребоваться управлять уровнем шифрования для повышения уровня безопасности (AES-256 надежнее, чем AES-128). Если этот параметр включен, можно настраивать алгоритм шифрования и стойкость шифра ключа для жестких дисков с данными, дисков операционной системы и съемных носителей с данными по отдельности. Для жестких дисков и дисков операционной системы рекомендуется использовать алгоритм XTS-AES. Для съемных носителей следует использовать алгоритм "AES-CBC, 128 бит" или "AES-CBC, 256 бит", если носитель будет использоваться на других устройствах, работающих не под управлением Windows 10 версии 1511 или более поздней. Изменение метода шифрования не оказывает никакого влияния, если диск уже зашифрован или находится в процессе шифрования. В этом случае данный параметр политики игнорируется.  

   Для политики диска системы locker бит настройте следующие параметры:
  - **Метод шифрования**  
    **По умолчанию**: AES 256-разрядный XTS  
...
- **Credential Guard**  
  ...
  **По умолчанию**: Включение с блокировкой UEFI 
...
- **Hardware device installation by device identifiers** (Установка аппаратных устройств по кодам устройств)  
  ...
  **По умолчанию**: блокировать установку аппаратных устройств  
    ...
    Когда *блокирует установку оборудования устройства* — флажок установлен, доступны следующие параметры.
...
- **Require password** (Требовать пароль)  
  ...
  **По умолчанию**: Да  
    ...
    Когда *требовать пароль* присваивается *Да*, доступны следующие параметры.

    - **Password minimum character set count** (Минимальное число наборов символов для пароля)  
      Число типов сложных элементов (прописные и строчные буквы, цифры и знаки пунктуации) для надежного ПИН-кода или пароля. ПИН-код обеспечивает следующее поведение для настольных и мобильных устройств: 1 - цифр только 2 - цифры и строчные буквы, требуется 3 - цифр, строчных букв и прописные необходимы. Не поддерживается в учетных записях Майкрософт для настольных систем и учетных записях домена. 4 — требуются цифры, строчные буквы, прописные буквы и специальные знаки. Не поддерживается на настольных системах. Значение по умолчанию — 1. 
    ...
    - **Требуемый тип пароля**  
      ...
      **По умолчанию**: буквенно-цифровых  
  ...
  - **Запретить использование предыдущих паролей**  
    Указывает, сколько паролей, недоступных для использования, вмещает журнал. Это значение учитывает текущий пароль пользователя. Например, с параметром *1* пользователю нельзя повторно использовать их текущий пароль, при выборе нового пароля. Значение *5* означает, что пользователь не может задать новый пароль их текущий пароль или любой из четырех предыдущие пароли.
    
    **По умолчанию**: 24  

- **Prevent slide show** (Запретить слайд-шоу)  
  Отключает параметры слайд-шоу для экрана блокировки в параметрах компьютера и запрещает воспроизведение слайд-шоу на этом экране. По умолчанию пользователи могут включить слайд-шоу, воспроизводимое после блокировки компьютера. Если этот параметр включен, пользователи не могут изменять параметры слайд-шоу в параметрах ПК, и можно начать слайд-шоу.
...
- **Блокировать Windows: интересное**  
  ...
  **По умолчанию**: Да  
  ...
  Когда *блок Windows Spotlight* присваивается *Да*, доступны следующие параметры.
...
- **XML защиты от эксплойтов**  
  ...
  **По умолчанию**: *предоставляется образца xml* 
...
- **Internet Explorer обходить смарт-экрана предупреждения о нестандартных файлов**  
...
- **Internet Explorer ограниченными разрешениями зоны java**  
...
- **Internet Explorer restricted zone logon options** (Параметры входа для ограниченной зоны Internet Explorer)  
  ...
  - *Автоматический вход только в зону интрасети* -используйте этот параметр, чтобы запрос пользователей для идентификаторов пользователей и пароли в других зонах. После выполнения запроса эти значения могут повторно использоваться автоматически в течение текущего сеанса. 
  - *Автоматический вход с помощью текущего имени пользователя и пароль*-используйте этот параметр для попытки входа с помощью Windows NT вызов-ответ (также называемую проверкой подлинности NTLM). Если сервер поддерживает ответ на запрос Windows NT, вход пользователя сетевое имя пользователя и пароль для входа используется. Если сервер не поддерживает запрос и подтверждение Windows NT, то пользователю нужно ввести имя и пароль. 
...
- **Зоны не выполняются, Internet Explorer интрасети защиты от вредоносных программ от элементов управления Active X** этот параметр политики определяет, выполняет ли Internet Explorer антивредоносные программы для элементов ActiveX, чтобы проверить, если они являются безопасными для загрузки страниц. Если вы включаете этот параметр политики, Internet Explorer не проверяет с помощью антивредоносного ПО, безопасно ли создать экземпляр элемента ActiveX. Если вы отключаете этот параметр политики, Internet Explorer всегда проверяет с помощью антивредоносного ПО, безопасно ли создать экземпляр элемента ActiveX. Если вы не настраиваете этот параметр политики, Internet Explorer не проверяет с помощью антивредоносного ПО, безопасно ли создать экземпляр элемента ActiveX. Пользователи могут включать и отключать это поведение с помощью параметров безопасности Internet Explorer
...
- **Internet Explorer internet zone updates to status bar via script** (Обновление строки состояния с помощью сценария для зоны Интернета Internet Explorer)  
  Этот параметр политики позволяет управлять возможностью сценария обновлять строку состояния в пределах зоны. Если этот параметр включен, скрипты можно обновить в строке состояния. Если этот параметр политики отключен или не настроен, сценарию запрещено обновлять строку состояния.
...
- **Internet Explorer users changing policies** (Изменение политик пользователями в Internet Explorer)  
    Запрещает пользователям изменять параметры зон безопасности. Зона безопасности — это группа веб-сайтов с одинаковым уровнем безопасности. Если этот параметр политики включен, кнопка "Другой" и ползунок уровня безопасности на вкладке "Безопасность" диалогового окна свойств браузера будут отключены. Если этот параметр политики отключен или не настроен, пользователи могут изменять параметры зон безопасности. Этот параметр политики не позволяет пользователям изменять параметры зон безопасности, установленные администратором. Примечание. Параметр политики "Отключить вкладку «Безопасность»" (в разделе \Конфигурация пользователя\Административные шаблоны\Компоненты Windows\Internet Explorer\Панель управления браузером), удаляющий вкладку "Безопасность" из Internet Explorer на панели управления, имеет приоритет над этим параметром политики. Если он включен, то данный параметр политики не учитывается. Кроме того, см. в разделе «зоны безопасности: использовать только параметры компьютера» политики.
...
- **Запретить анонимный доступ к именованным каналам и общим ресурсам**  
  Если этот параметр включен, этот параметр безопасности ограничивает анонимный доступ к общим ресурсам и каналам для: (1) именованные каналы, которые могут быть анонимный доступ к (2) общие папки, которые может осуществляться анонимно
  
  **По умолчанию**: Да  
  
- **Minimum session security for NTLM SSP based servers** (Минимальная сеансовая безопасность для серверов на базе NTLM SSP)  
  Этот параметр безопасности позволяет серверу требовать согласование 128-разрядного шифрования и (или) сеансовой безопасности NTLMv2. Эти значения зависят от значения параметра безопасности "Уровень проверки подлинности LAN Manager". Возможные варианты:: Требовать сеансовую безопасность NTLMv2: соединение завершится ошибкой, если целостность сообщения не согласован. "Требовать 128-битное шифрование". Если стойкое (128-разрядное) шифрование не согласовано, подключение не будет установлено.
...
- **Уровень проверки подлинности**  
  ...
  - *Отправлять только ответ NTLM*: клиенты используют только проверку подлинности NTLM, а также сеансовую безопасность NTLMv2, если сервер ее поддерживает; контроллеры домена принимают проверку подлинности LM, NTLM и NTLMv2. 
  ...
  - *Отправлять только ответ NTLMv2. Отклонять LM и NTLM*: клиенты используют только проверку подлинности NTLMv2, а также сеансовую безопасность NTLMv2, если сервер ее поддерживает; контроллеры домена отклоняют LM и NTLM (принимая только проверку подлинности NTLMv2). 
...
- **Administrator elevation prompt behavior** (Поведение запроса на повышение прав для администраторов)  
  Этот параметр политики определяет поведение запроса на повышение прав для администраторов. Доступны следующие параметры. 
    - *Повышение без запроса*. Привилегированным учетным записям разрешено выполнение операций, требующих повышения прав, без подтверждения согласия или ввода учетных данных. Примечание. Этот вариант должен использоваться только в средах с максимальными ограничениями. 
...
- **Minimum session security for NTLM SSP based clients** (Минимальная сеансовая безопасность для клиентов на базе NTLM SSP)  
  ...
  - Требовать сеансовую безопасность NTLMv2: соединение завершится ошибкой, если протокол NTLMv2 не согласован. 
  - *Требуется 128-битное шифрование*: соединение завершится ошибкой, если не согласовано стойкого шифрования (128-разрядное).
...
- **Поведение при извлечении смарт-карты**  
    Этот параметр безопасности определяет, что происходит при извлечении смарт-карты вошедшего пользователя из устройства чтения смарт-карт. Доступны следующие параметры.
     - *Никаких действий*. 
     - *Блокировки рабочей станции* -рабочая станция блокируется при удалении смарт-карты, позволяя пользователям область, выполните их смарт-карты с ними и по-прежнему поддерживать защищенный сеанс.
...
- **Standard user elevation prompt behavior** (Поведение запроса на повышение прав для обычных пользователей)  
  ...
  - *Запрос учетных данных на безопасном рабочем столе*. Для любой операции, требующей повышения прав, на безопасном рабочем столе выводится приглашение ввести другие имя и пароль пользователя. Если пользователь вводит правильные учетные данные, операция будет продолжена с соответствующими привилегиями. 
  - *Запрос учетных данных*. Для любой операции, требующей повышения прав, выводится приглашение ввести имя пользователя и пароль учетной записи администратора. Если пользователь вводит правильные учетные данные, операция будет продолжена с соответствующими привилегиями.  
...
- **Allow remote calls to security accounts manager** (Разрешить удаленные вызовы диспетчера учетных записей безопасности)  
  ...
  **По умолчанию**: *O:BAG:BAD:(A;; ВЕРСИЯ-КАНДИДАТ;; БИЗНЕС-АНАЛИТИКИ)*
...
- **Обнаружение установки приложений и запрос на повышение прав**  
  ...
  - *Включено*: при обнаружении установочного пакета приложения, которому требуется повышение прав, пользователю предлагается ввести имя пользователя и пароль учетной записи администратора. Если пользователь вводит правильные учетные данные, операция будет продолжена с соответствующими привилегиями. 
...
- **Network IP source routing protection level** (Уровень защиты исходной маршрутизации сетевых IP-адресов)  
  **По умолчанию**: самый высокий защиты  
...
- **Standby states when sleeping while on battery** (Ждущие режимы в спящем режиме при работы от батареи)  
  Этот параметр политики управляет, если Windows использовать ждущие режимы, когда компьютер переводится в режим сна. Если вы включите или не настроите этот параметр политики, Windows будет использовать ждущие режимы. Если вы отключите этот параметр политики, ждущие режимы (S1–S3) будут запрещены.
...
- **RPC unauthenticated client options** (Параметры для непроверенных RPC-клиентов)  
  ...
  **По умолчанию**: проверка подлинности
...
## <a name="smart-screen"></a>Экран Smart  
...
- **System boot start driver initialization** (Инициализация драйверов при загрузке ОС)  
  ...
  - *Неудовлетворительно*: драйвер был определен как вредоносная программа. Мы рекомендуем не допускать известных неправильный драйверы должны быть инициализированы. 
...
- **Email content execution type** (Тип выполнения содержимого электронной почты)  
  Это правило блокирует следующие типы файлов из процесса запуска или запускается из сообщения электронной почты в Microsoft Outlook или веб-почты (например, Gmail.com и Outlook.com): исполняемые файлы (.exe, .dll или .scr) файлы скриптов (например, вводить PowerShell, VisualBasic .vbs, или JavaScript js-файл) архивных файлов скрипта.
  
  **По умолчанию**: блок
  
- **Network protection type** (Тип защиты сети)  
  Эта политика позволяет включить или отключить защиту сети (блокировка/аудит) в Exploit Guard в Защитнике Windows. Функция защиты сети входит в состав Exploit Guard в Защитнике Windows и защищает сотрудников, использующих любое приложение, от фишинговых сообщений, сайтов с эксплойтами и вредоносного содержимого в Интернете. Кроме того, она не позволяет сторонним браузерам подключаться к опасным сайтам. Тип значения — целое число. Если этот параметр включен, защита сети включена, и сотрудники не могут ее отключить. Работой этой функции можно управлять с помощью следующих параметров: "Блокировка" и "Аудит". Если включить эту политику с параметром "Блокировка", пользователи и приложения не смогут подключиться к опасным доменам. Вы увидите это действие в центре безопасности Защитника Windows. Если включить эту политику с параметром "Аудит", пользователи и приложения смогут подключиться к опасным доменам. При этом вы также можете просматривать соответствующую активность в Центре безопасности Защитника Windows. Если отключить эту политику, пользователи и приложения смогут подключиться к опасным доменам. При этом никакая сетевая активность в Центре безопасности Защитника Windows не отображается Если не настраивать эту политику, сети блокируются отключен по умолчанию.
...
- **Defender potentially unwanted app action** (Действие для потенциально нежелательных приложений в Защитнике)  
  Функция защиты от потенциально нежелательных приложений (PUA) в антивирусной программе "Защитник Windows" позволяет идентифицировать и блокировать скачивание и установку таких приложений на конечных точках в вашей сети. Эти приложения не относятся к вирусам, вредоносным программам и другим видам угроз, однако могут выполнять на конечных точках действия, отрицательно влияющие на производительность или удобство использования. Понятие PUA также может относиться к приложениям, которые, как считается, обладают плохой репутацией. Типичное поведение PUA включает в себя: различные типы программного обеспечения, объединение внедрение Ad в веб-браузерах драйвера и Оптимизаторы реестра, которые позволяют обнаружить проблемы, запрос оплаты для исправления ошибок, но остаются в конечной точке и сделать без изменения или оптимизации (также называется» программы незаконных антивирусные»). Эти приложения можно повысить риск сети заражения вредоносными программами, вызывают гораздо сложнее определить заражении вредоносными программами, а приведут к трате ИТ-ресурсы в очистке приложений.  
...
- **Контроль поведения**  
  Разрешает или запрещает функцию контроля поведения Защитника Windows. Эти встроенные в Windows 10 датчики собирают и обрабатывают сигналы о действиях, поступающие из операционной системы, а затем отправляют эти данные в частный изолированный облачный экземпляр ATP в Защитнике Windows.
...
- **Ink Workspace**  
  ...
  - *Не настроено* - функция Ink Workspace включена, и пользователь может использовать его поверх экрана блокировки.  
