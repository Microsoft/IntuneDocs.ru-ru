...
-  **Соединитель сертификатов Microsoft Intune**: используйте портал Intune, чтобы скачать установщик **соединителя сертификатов** (**ndesconnectorssetup.exe**). Затем вы можете запустить **ndesconnectorssetup.exe** на компьютере, где требуется установить соединитель сертификатов. 
...
> [!NOTE]
> Из-за известной проблемы скачивать, устанавливать и настраивать соединитель сертификатов, следует с помощью следующей процедуры: [Настройка инфраструктуры сертификата для SCEP -> Настройка инфраструктуры -> Задача 5](/intune-classic/deploy-use/configure-certificate-infrastructure-for-scep)
