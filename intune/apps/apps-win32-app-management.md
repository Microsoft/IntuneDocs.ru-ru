...
3. Выберите **Приложения** > **Все приложения** > **Добавить**.
4. На панели **Добавить приложение** выберите в раскрывающемся списке **Приложение Windows (Win32)** .

    ![Снимок экрана с панелью "Добавление приложения" и раскрывающимся списком "Добавить тип"](./media/apps-win32-app-management/apps-win32-app-01.png)
...
1. В панели **Добавить приложение** выберите **Файл пакета приложения**, чтобы выбрать файл. Появится панель файла пакета приложения.
    ...
    ![Снимок экрана с панелью "Файл пакета приложения"](./media/apps-win32-app-management/apps-win32-app-02.png)
...
2. Чтобы настроить **команду установки**, добавьте полную строку команды для установки приложения. 
...
4. Для параметра **Способ установки** укажите значение **Система** или **Пользователь**.
...
5. Чтобы настроить **Действие при перезагрузке устройства**, выберите один из следующих вариантов:
    - **Определять режим по кодам возврата**. Выберите этот вариант, чтобы перезапускать устройство по параметрам [кодов возврата](~/apps/apps-win32-app-management.md#step-7-configure-app-return-codes).
...
6. Завершив настройку, нажмите кнопку **ОК**.
...
5. На панели **Назначение** выберите пользователей или устройства, которым нужно назначить приложение. При выборе назначений можно также выбрать **Взаимодействие с пользователем**. Вариант **Взаимодействие с пользователем** позволяет задать **Уведомления для конечных пользователей**, **Период отсрочки до перезапуска**, **Доступность** и **Крайний срок установки**. Дополнительные сведения см. в разделе **Настройка доступности и уведомлений для приложений Win32**.
6. Щелкните **ОК**, чтобы завершить выбор включенных групп.
7. Щелкните **Исключить группы**, если вы хотите исключить из назначения приложения какие-либо группы пользователей.
8. На панели **Добавить группу** нажмите кнопку **OK**.
9. На панели **Назначения** выберите **Сохранить**.
...
1. В Intune выберите **Приложения** > **Все приложения**, чтобы просмотреть список добавленных клиентских приложений. 
...
4. На панели этого приложения щелкните **Назначения** > **Добавить группу**. 
5. Установите значение параметра **Тип назначения** на **Обязательно**. Обратите внимание, что доступность приложения может зависеть от типа назначения. В списке **Тип назначения** можно выбрать **Требуется**, **Доступно для зарегистрированных устройств** или **Удалить**.
6. Выберите **Включенные группы**, чтобы указать группу пользователей, которой будет назначено это приложение. Отобразится панель **Назначить**.
7. Для параметра **Сделать это приложение обязательным для всех пользователей** выберите значение **Да**.

    > [!NOTE]
    > Для параметра **Тип назначения** доступны следующие варианты:<br>
    > - **Обязательно**. Вы можете **сделать это приложение обязательным для всех пользователей** и (или) **сделать это приложение обязательным на всех устройствах**.<br>
...
13. Щелкните **ОК** > **ОК** > **ОК** > **Сохранить**, чтобы добавить назначение.
