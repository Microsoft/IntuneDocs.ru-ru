...
2. Если вам нужно вызвать любой из интерфейсов API Intune из SWIFT, ваше приложение или расширение должно импортировать необходимые заголовки пакета SDK Intune через заголовок моста цели-C. Если ваше приложение или расширение еще не содержит заголовок моста "цель-C", можно указать его с `SWIFT_OBJC_BRIDGING_HEADER` помощью параметра конфигурации сборки или поля заголовка Xcode пользовательского интерфейса " **Целевая привязка** ". Заголовок "мост" должен выглядеть примерно так:

   ```objc
      #import <IntuneMAMSwift/IntuneMAM.h>
   ```
   
   Это сделает все интерфейсы API пакета SDK для Intune доступными во всех исходных файлах SWIFT вашего приложения или расширения. 
   
    > [!NOTE]
    > * Вы можете выбрать только мосты для конкретных заголовков пакета SDK Intune, а не все, что охватывает IntuneMAM. h.
    > * В зависимости от того, какая платформа или статическая библиотека интегрирована, путь к файлам заголовков может отличаться.
    > * Предоставление доступа к API пакета SDK Intune в SWIFT с помощью инструкции import модуля (например, Import Интунемамсвифт) в настоящее время не поддерживается. Рекомендуемым подходом является использование коннектора с мостом "цель-C".
...
4. Включите совместное использование цепочки ключей (если оно еще не включено), щелкнув **Capabilities** (Возможности) в каждой цели проекта и включив параметр **Keychain Sharing** (Совместное использование цепочки ключей). Совместное использование цепочки ключей требуется для перехода к следующему шагу.
...
7. Если ваше приложение еще не использует FaceID, убедитесь, что для [ключа NSFaceIDUsageDescription Info.plist](https://developer.apple.com/library/archive/documentation/General/Reference/InfoPlistKeyReference/Articles/CocoaKeys.html#//apple_ref/doc/uid/TP40009251-SW75) настроено сообщение по умолчанию. Это необходимо, чтобы iOS могла сообщить пользователю о том, как приложение собирается использовать FaceID. Настройка политики защиты приложений Intune позволяет использовать FaceID в качестве метода для доступа к приложению, если он настроен ИТ-администратором.

8. Используйте средство IntuneMAMConfigurator, включенное в [репозиторий SDK](https://github.com/msintuneappsdk/ms-intune-app-sdk-ios), для завершения настройки приложения в файле Info.plist. У средства есть три параметра:
...
сафаривиевконтроллерблоккедоверриде | Логическое значение| Отключает перехватчики Сафаривиевконтроллер Intune для включения проверки подлинности MSAL через Сфсафаривиевконтроллер, Сфауссессион или Асвебауссессион. | Необязательный параметр. По умолчанию используется значение No (Нет). Предупреждение: может привести к утечке данных при неправильном использовании. Включайте только в случае крайней необходимости. Дополнительные сведения см. [в разделе Специальные рекомендации при использовании MSAL](#special-considerations-when-using-msal) .  |
