...
* Потребуется компьютер под управлением Mac OS X 10.8.5 или более поздней версии, на котором установлен XCode версии 8 или более поздней.
...
4. Если ваше мобильное приложение определяет главный NIB- или STORYBOARD-файл в своем файле Info.plist, удалите поле **Main Storyboard** или **Main Nib**. В файле Info.plis вставьте эти поля и их соответствующие значения в новый словарь **IntuneMAMSettings** со следующими названиями ключей (если применимо):
    * MainStoryboardFile
...
5. Включите совместное использование цепочки ключей (если оно еще не включено), щелкнув **Capabilities** (Возможности) в каждой цели проекта и включив параметр **Keychain Sharing** (Совместное использование цепочки ключей). Совместное использование цепочки ключей требуется для перехода к следующему шагу.
...
6. Чтобы создать отдельную группу доступа, в которой будут храниться данные пакета SDK для приложений Intune, после включения совместного использования цепочки ключей сделайте следующее. Группу доступа к цепочке ключей можно создать при помощи пользовательского интерфейса или файла прав. При использовании пользовательского интерфейса для создания группы доступа к цепочке ключей обязательно выполните следующие действия:
    ...
    4. Если вы изменяете файл прав непосредственно (не с помощью пользовательского интерфейса Xcode, показанного выше) для создания группы доступа к цепочке ключей, добавьте `$(AppIdentifierPrefix)` в начало групп доступа к цепочке ключей (Xcode обрабатывает это значение автоматически). Например:

            * `$(AppIdentifierPrefix)com.microsoft.intune.mam`
            * `$(AppIdentifierPrefix)com.microsoft.adalcache`

    > [!NOTE]
    > Файл прав — это XML-файл, который является уникальным в рамках вашего мобильного приложения. Он используется для задания специальных разрешений и возможностей в вашем приложении для iOS. Если файла прав нет, включите общий доступ к цепочке ключей (шаг 6), чтобы это файл был создан в Xcode для вашего приложения.
...
8. Если для приложения в файле Info.plist определены типы документов, добавьте к массиву «UTI типов содержимого документа» каждого элемента дублирующую запись для каждой строки с префиксом «com.microsoft.intune.mam.».
...
3. В словаре **IntuneMAMSettings** с именем ключа `ADALRedirectUri` также укажите URI перенаправления, который будет использоваться для вызовов ADAL. В зависимости от формата URI перенаправления приложения может также потребоваться указать `ADALRedirectScheme`.


Кроме того, вы можете переопределить URL-адрес центра сертификации Azure AD, используя URL-адрес определенного клиента во время выполнения. Чтобы сделать это, просто определите свойство `aadAuthorityUriOverride` для экземпляра `IntuneMAMPolicyManager`.
...
В словаре IntuneMAMSettings можно добавить строки типа "ключ — значение" с параметрами конфигурации для настройки пакета SDK. В таблице ниже приведен список всех поддерживаемых параметров.
...
3. В словаре **IntuneMAMSettings** в файле Info.plist приложения добавьте логическое значение DebugSettingsEnabled. Для DebugSettingsEnabled следует задать значение Yes.
...
| -- | -- | -- |
| AccessRecheckOfflineTimeout | Время в минутах, в течение которого приложение может выполняться в автономном режиме, прежде чем Intune заблокирует для приложения возможность запуска или возобновления работы, если включена проверка подлинности. | Любое целое число больше 0 |
|   AccessRecheckOnlineTimeout | Время в минутах, в течение которого приложение может выполняться, прежде чем пользователю потребуется ввести ПИН-код или выполнить проверку подлинности при запуске или возобновлении работы приложения (если для доступа включены проверка подлинности или использование ПИН-кода). | Любое целое число больше 0 |
