...
## <a name="whats-in-the-sdk-repository"></a>Что такое в репозитории пакетов SDK

В приложениях или расширения, которые не содержат Swift кода или скомпилированные с использованием версии Xcode, прежде чем 10.2 относятся следующие файлы:

* **IntuneMAM.framework**: платформа пакета SDK для приложений Intune. Рекомендуется, свяжите эту платформу приложения или расширения для управления приложениями клиента Intune. Однако некоторым разработчикам может предпочесть преимущества производительности статическую библиотеку. См. следующие ресурсы:

* **libIntuneMAM.a**: статическая библиотека пакета SDK для приложений Intune. Разработчики могут выбрать для связывания со статической библиотекой вместо framework. Поскольку статические библиотеки внедренные непосредственно в двоичный во время сборки приложения и расширения, существуют некоторые преимущества производительности во время запуска при использование статической библиотеки. Тем не менее его интеграции в веб-приложения является более сложным процессом. Если приложение включает любые расширения, компоновка со статической библиотекой для приложения и расширения приведет к увеличению размера пакета приложения, как статическая библиотека будут внедрены в каждое приложение/расширение двоичный. При использовании платформы, приложения и расширения могут совместно использовать тот же двоичный пакет SDK для Intune, что позволяет уменьшить размер приложения.

* **IntuneMAMResources.bundle:** набор ресурсов, на которых основан пакет SDK. Ресурсы пакета является обязательным только для приложений, которые интегрируются со статической библиотекой (libIntuneMAM.a).

В приложениях или расширения, содержащие код Swift и скомпилированные с использованием Xcode 10.2 + относятся следующие файлы:

* **IntuneMAMSwift.framework**: The Intune App SDK Swift framework. Эта платформа содержит все заголовки для интерфейсов API, который вызывает приложение. Свяжите эту платформу приложения или расширения для управления приложениями клиента Intune.

* **IntuneMAMSwiftStub.framework**: The Intune App SDK Swift заглушки framework. Это связано с необходимой зависимости из IntuneMAMSwift.framework, который необходимо связать приложения или расширения.


Для всех приложений и расширений относятся следующие файлы:

* **IntuneMAMConfigurator**: это средство используется для настройки приложения или Info.plist расширения с минимальным изменений для системы управления Intune. В зависимости от функциональности приложения или расширения может потребоваться внести дополнительные изменения вручную в файле Info.plist.

* **Заголовки:** предоставляет API пакета SDK для приложений Intune. Эти заголовки включаются в IntuneMAM/IntuneMAMSwift платформ, поэтому разработчики, которые используют любой из платформ не нужно вручную добавить заголовки в своих проектах. Разработчики, которые выбраны для связывания со статической библиотекой (libIntuneMAM.a) необходимо вручную включить эти заголовки в своих проектах.
...
1. **Вариант 1. (рекомендуется) Framework**: Если вы используете Xcode 10.2 + и приложения/расширения содержит код Swift, свяжите `IntuneMAMSwift.framework` и `IntuneMAMSwiftStub.framework` с целевыми объектами: перетащите `IntuneMAMSwift.framework` и `IntuneMAMSwiftStub.framework` для **внедренный Двоичные файлы** список цели проекта.

    В противном случае связать `IntuneMAM.framework` с целевыми объектами: перетащите `IntuneMAM.framework` для **Embedded двоичные файлы** список цели проекта.
   ...
   **Вариант 2 - статическая библиотека**: этот параметр доступен только для приложений и расширений, которые не содержат Swift кода или созданными с помощью Xcode < 10.2. Выполните связывание с библиотекой `libIntuneMAM.a`. Перетащите библиотеку `libIntuneMAM.a` в список **Linked Frameworks and Libraries** (Связанные платформы и библиотеки) цели проекта.
...
2. Если вам нужно вызывать любую из интерфейсам API Intune из Swift, приложения и расширения необходимо импортировать требуемые заголовки пакета SDK для Intune через промежуточный заголовок Objective-C. Если приложение или расширение не включает промежуточный заголовок Objective-C, можно указать один через `SWIFT_OBJC_BRIDGING_HEADER` параметр конфигурации сборки или пользовательского интерфейса Xcode **заголовок Objective-C** поля. Ваши заголовок должен выглядеть следующим образом:

   ```objc
      #import <IntuneMAMSwift/IntuneMAM.h>
   ```
   
   Это будет доступен все Intune пакета SDK API-интерфейсы на протяжении всех Swift исходных файлов приложения и расширения. 
   
    > [!NOTE]
    > * Вы можете только моста конкретного пакета SDK для Intune заголовки Swift, а не IntuneMAM.h охватывает все
    > * В зависимости от библиотеки framework (или) статического интегрированных путь к файлам заголовка может отличаться.
    > * Предоставление API пакета SDK для Intune в Swift с помощью инструкции импорта модуля (например: Импорт IntuneMAMSwift) сейчас не поддерживается. Использование моста заголовок Objective-C является рекомендуемым подходом.
...
4. Включите единый вход ADAL, добавив `com.microsoft.adalcache` к группам доступа ключей.
...
MAMPolicyRequired| Логическое значение| Указывает, будет ли блокироваться запуск приложения без политики приложения Intune. По умолчанию используется значение NO (Нет). <br><br> Примечание. В магазин приложений нельзя отправить приложения, у которых параметр MAMPolicyRequired имеет значение YES. | Необязательный параметр. По умолчанию используется значение No (Нет).|
MAMPolicyWarnAbsent | Логическое значение| Указывает, будет ли приложение без политики приложений Intune уведомлять пользователя во время запуска. <br><br> Примечание. Пользователи по-прежнему смогут использовать приложение без политики, отклонив предупреждение. | Необязательный параметр. По умолчанию используется значение No (Нет). |
