...
5. Если ваше мобильное приложение определяет главный NIB- или STORYBOARD-файл в своем файле Info.plist, удалите поле **Main Storyboard** или **Main Nib**. В файле Info.plis вставьте эти поля и их соответствующие значения в новый словарь **IntuneMAMSettings** со следующими названиями ключей (если применимо):
    * MainStoryboardFile
...
7. Чтобы создать отдельную группу доступа, в которой будут храниться данные пакета SDK для приложений Intune, после включения совместного использования цепочки ключей сделайте следующее. Группу доступа к цепочке ключей можно создать при помощи пользовательского интерфейса или файла прав. При использовании пользовательского интерфейса для создания группы доступа к цепочке ключей обязательно выполните следующие действия:
    ...
    5. Если для создания группы доступа к цепочке ключей используется файл прав, добавьте `$(AppIdentifierPrefix)` в начало этой группы в файле прав. Пример.
