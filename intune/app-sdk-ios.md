...
2. Включите единый вход ADAL, добавив `com.microsoft.adalcache` к группам доступа ключей.

3. Если вы хотите задать настраиваемую группу цепочки ключей для замены `com.microsoft.adalcache`, укажите ее в разделе "IntuneMAMSettings" файла Info.plist, используя ключ `ADALCacheKeychainGroupOverride`.
...
## <a name="receiving-app-protection-policy"></a>Получение политики защиты приложений

### <a name="overview"></a>Обзор
Чтобы получить политику защиты приложения Intune, приложения должны запустить запрос на регистрацию в службе Intune. В консоли Intune можно настроить приложения для получения политики защиты с регистрацией устройства или без нее. Политика защиты приложений без регистрации, также известная как **APP-WE** или MAM-WE, позволяет управлять приложениями с помощью Intune без необходимости регистрации устройств в ПО управления мобильными устройствами (MDM) Intune. В обоих случаях для получения политики требуется регистрация в службе Intune.
...
Этот метод должен вызываться до удаления токенов Azure AD учетной записи пользователя. Пакету SDK требуется, чтобы токены AAD учетной записи пользователя выполняли определенные запросы к службе Intune от имени пользователя.
