...
* **libIntuneMAM.a**: статическая библиотека пакета SDK для приложений Intune. Разработчики могут связать статическую библиотеку, а не платформу. Поскольку статические библиотеки внедряются непосредственно в двоичный файл приложения или расширения во время сборки, для использования статической библиотеки существует ряд преимуществ производительности во время запуска. Однако интеграция этого приложения в приложение является более сложным процессом. Если приложение содержит расширения, связывание статической библиотеки с приложением и расширения приведет к увеличению размера набора приложений, так как статическая библиотека будет внедрена в каждый двоичный файл приложения или расширения. При использовании платформы приложения и расширения могут совместно использовать один и тот же двоичный файл пакета SDK Intune, что приводит к уменьшению размера приложения.

* **IntuneMAMResources.bundle:** набор ресурсов, на которых основан пакет SDK. Набор ресурсов требуется только для приложений, которые интегрируют статическую библиотеку (libIntuneMAM. a).

Следующие файлы относятся к приложениям и расширениям, содержащим код SWIFT, и компилируются с помощью Xcode 10.2 +:

* **Интунемамсвифт. Framework**: структура SWIFT пакета SDK для приложений Intune. Эта платформа содержит все заголовки для API-интерфейсов, которые будут вызываться приложением. Свяжите эту платформу с приложением или расширениями, чтобы включить управление клиентскими приложениями Intune.

* **Интунемамсвифтстуб. Framework**: инфраструктура-заглушка пакета SDK для приложений Intune. Это обязательная зависимость от Интунемамсвифт. Framework, с которой должны быть связаны приложения или расширения.


Следующие файлы относятся ко всем приложениям и экстентам:

* **IntuneMAMConfigurator**: средство, используемое для настройки сведений о приложении или расширении. plist с минимально необходимыми изменениями для управления Intune. В зависимости от функциональных возможностей приложения или расширения, может потребоваться внести дополнительные изменения вручную в info. plist.

* **Заголовки:** предоставляет API пакета SDK для приложений Intune. Эти заголовки включены в платформы IntuneMAM/Интунемамсвифт, поэтому разработчикам, использующим любую из этих платформ, не нужно вручную добавлять заголовки в свой проект. Разработчикам, которые выбирают связь со статической библиотекой (libIntuneMAM. a), потребуется вручную включить эти заголовки в свой проект.
...
1. **Вариант 1 — платформа (рекомендуется)** . Если вы используете Xcode 10.2 +, а приложение или расширение содержит код SWIFT `IntuneMAMSwift.framework` , свяжите и `IntuneMAMSwiftStub.framework` целевой объект: перетащите `IntuneMAMSwift.framework` и `IntuneMAMSwiftStub.framework` в список внедренных **двоичных файлов** Целевой объект проекта.

    В противном `IntuneMAM.framework` случае свяжите с целевым объектом `IntuneMAM.framework` : перетащите в список **внедренные двоичные файлы** для целевого объекта проекта.
   ...
   **Вариант 2. Статическая библиотека**. Этот параметр доступен только для приложений и расширений, которые не содержат SWIFT-кода, или были созданы с помощью Xcode < 10,2. Выполните связывание с библиотекой `libIntuneMAM.a`. Перетащите библиотеку `libIntuneMAM.a` в список **Linked Frameworks and Libraries** (Связанные платформы и библиотеки) цели проекта.
...
4. Чтобы создать отдельную группу доступа, в которой будут храниться данные пакета SDK для приложений Intune, включив совместное использование цепочки ключей, выполните приведенные ниже инструкции. Группу доступа к цепочке ключей можно создать при помощи пользовательского интерфейса или файла прав. При использовании пользовательского интерфейса для создания группы доступа к цепочке ключей обязательно выполните следующие действия:
    ...
    d. Если вы изменяете файл прав непосредственно (не с помощью пользовательского интерфейса Xcode, показанного выше) для создания группы доступа к цепочке ключей, добавьте `$(AppIdentifierPrefix)` в начало групп доступа к цепочке ключей (Xcode обрабатывает это значение автоматически). Например:
...
Рекомендуется, чтобы ваше приложение собыло ссылкой на последнюю версию [ADAL](https://github.com/AzureAD/azure-activedirectory-library-for-objc/releases) или [MSAL](https://github.com/AzureAD/microsoft-authentication-library-for-objc/releases).
...
**Вариант 2** . Кроме того, вы можете выполнить [эти инструкции](https://github.com/AzureAD/microsoft-authentication-library-for-objc#installation) , чтобы связать приложение с двоичными файлами MSAL.
...
Как упоминалось ранее, пакет SDK для приложений Intune может использовать либо [библиотеку проверки Подлинности Azure Active Directory](https://github.com/AzureAD/azure-activedirectory-library-for-objc) , либо [библиотеку проверки подлинности Майкрософт](https://github.com/AzureAD/microsoft-authentication-library-for-objc) для сценариев проверки подлинности и условного запуска. Пакет SDK также использует ADAL/MSAL для регистрации удостоверения пользователя в службе MAM для управления без сценариев регистрации устройства. Если **приложение не использует ADAL или MSAL для собственного механизма проверки подлинности**, может потребоваться настроить НАСТРАИВАЕМЫЕ параметры AAD в зависимости от выбранной библиотеки проверки подлинности.   

ADAL — пакет SDK для приложений Intune предоставит значения по умолчанию для параметров ADAL и будет обрабатывать операции проверки подлинности в Azure AD. Разработчикам не нужно указывать какие бы то ни было значения для указанных ранее параметров ADAL. 

MSAL — разработчики должны создать регистрацию приложения в AAD с помощью пользовательского URI перенаправления в указанном [здесь](https://github.com/AzureAD/microsoft-authentication-library-for-objc/wiki/Migrating-from-ADAL-Objective-C-to-MSAL-Objective-C#app-registration-migration)формате. Разработчикам следует задать `ADALClientID` ранее `ADALRedirectUri` упомянутые параметры и, а также `aadClientIdOverride` `IntuneMAMPolicyManager` эквивалент `aadRedirectUriOverride` и свойства экземпляра. Разработчики также должны убедиться, что они следуют за шагом 4 в предыдущем разделе, чтобы предоставить доступ к службе защиты приложений Intune для регистрации приложений.

### <a name="special-considerations-when-using-msal"></a>Специальные рекомендации при использовании MSAL 

1. **Проверьте WebView** . рекомендуется, чтобы приложения не использовали Сфсафаривиевконтроллер, Сфауссессион или асвебауссессион в качестве WebView для любых операций, инициированных приложением MSAL интерактивной аутентификации. Если по какой бы то ни было причине приложение должно использовать одно из этих веб – представлений для любых интерактивных операций MSAL AUTH, оно `SafariViewControllerBlockedOverride` также `true` должно быть `IntuneMAMSettings` указано в словаре в файле info. plist приложения. ПРЕДУПРЕЖДЕНИЕ. это приведет к отключению перехватчиков Сафаривиевконтроллер Intune для включения сеанса проверки подлинности. Это может привести к утечке данных в других местах приложения, если приложение использует Сафаривиевконтроллер для просмотра корпоративных данных, поэтому приложение не должно отображать корпоративные данные в любом из этих типов WebView.
2. **Связывание ADAL и MSAL** — разработчики должны принять участие в том случае, если требуется, чтобы INTUNE предпочитает MSAL через ADAL в этом сценарии. По умолчанию Intune будет предпочитать Поддерживаемые версии ADAL поддерживаемым версиям MSAL, если обе они связаны во время выполнения. Intune будет предпочитать поддерживаемую версию MSAL только в том случае, если во время первой операции `IntuneMAMUseMSALOnNextLaunch` проверки подлинности Intune находится `true` в `NSUserDefaults`. Если `IntuneMAMUseMSALOnNextLaunch` параметр `false` имеет значение или не задан, Intune будет возвращаться к поведению по умолчанию. Как видно из названия, изменение `IntuneMAMUseMSALOnNextLaunch` вступит в силу при следующем запуске.
...
--       |  --   |   --       |  --
ADALClientId  | Строка  | Идентификатор клиента Azure AD приложения. | Требуется для всех приложений, использующих MSAL, и любых приложений ADAL, которые обращаются к ресурсу AAD, отличному от Intune. |
ADALAuthority | Строка | Используется центр сертификации Azure AD приложения. Вам нужно использовать собственное окружение, где были настроены учетные записи AAD. | Требуется, если приложение использует ADAL или MSAL для доступа к ресурсу AAD, отличному от Intune. Если это значение отсутствует, используется значение по умолчанию для Intune.|
ADALRedirectUri  | Строка  | URI перенаправления Azure AD для приложения. | ADALRedirectUri или ADALRedirectScheme требуется для всех приложений, использующих MSAL, и любых приложений ADAL, которые обращаются к ресурсу AAD, отличному от Intune.  |
ADALRedirectScheme  | Строка  | Схема перенаправления Azure AD для приложения. Может использоваться вместо ADALRedirectUri, если URI перенаправления приложения имеет формат `scheme://bundle_id`. | ADALRedirectUri или ADALRedirectScheme требуется для всех приложений, использующих MSAL, и любых приложений ADAL, которые обращаются к ресурсу AAD, отличному от Intune. |
ADALLogOverrideDisabled | Логический  | Определяет, будет ли пакет SDK перенаправлять все журналы ADAL/MSAL (включая вызовы ADAL из приложения при их наличии) в свой файл журнала. По умолчанию — NO. Задайте значение YES (Да), если в приложении настроен собственный обратный вызов журнала ADAL/MSAL. | Необязательный параметр. |
...
DebugSettingsEnabled| Логический | Если задано значение "YES" (Да), можно применить политики теста в пакете параметров. В поставляемых приложения этот параметр *должен* быть отключен. | Необязательный параметр. По умолчанию используется значение No (Нет). |
...
AutoEnrollOnLaunch| Логический| Указывает, должно ли приложение автоматически регистрироваться при запуске, если приложение еще не зарегистрировано и если обнаружено существующее управляемое удостоверение. По умолчанию — NO. <br><br> Примечания. Если управляемое удостоверение не найдено или если в кэше ADAL/MSAL отсутствует действительный токен для удостоверения, то попытка регистрации завершится сбоем без сообщения об ошибке и без запроса учетных данных (за исключением случая, когда параметр приложения MAMPolicyRequired имеет значение YES). | Необязательный параметр. По умолчанию используется значение No (Нет). |
MAMPolicyRequired| Логический| Указывает, будет ли блокироваться запуск приложения при отсутствии политики защиты приложений Intune. По умолчанию используется значение NO (Нет). <br><br> Примечание. В Магазин приложений невозможно отправить приложения, в которых параметр MAMPolicyRequired имеет значение YES. При установке значения YES в параметр MAMPolicyRequired также следует установить значение YES в параметр AutoEnrollOnLaunch. | Необязательный параметр. По умолчанию используется значение No (Нет). |
MAMPolicyWarnAbsent | Логический| Указывает, будет ли приложение, для которого не настроена политика защиты приложений Intune, уведомлять пользователя во время запуска. <br><br> Примечание. Пользователи по-прежнему смогут использовать приложение без политики, отклонив предупреждение. | Необязательный параметр. По умолчанию используется значение No (Нет). |
MultiIdentity | Логический| Указывает, поддерживает ли приложение множество удостоверений. | Необязательный параметр. По умолчанию используется значение No (Нет). |
сафаривиевконтроллерблоккедоверриде | Логический| Отключает перехватчики Сафаривиевконтроллер Intune для включения проверки подлинности MSAL через Сфсафаривиевконтроллер, Сфауссессион или Асвебауссессион. | Необязательный параметр. По умолчанию используется значение No (Нет). Предупреждение: может привести к утечке данных при неправильном использовании. Включайте только в случае крайней необходимости. Дополнительные сведения см. [в разделе Специальные рекомендации при использовании MSAL](#special-considerations-when-using-msal) .  |
...
OpenInActionExtension | Логический | Задайте значение YES (Да) для расширений OpenInAction. Дополнительные сведения см. в разделе "Предоставление общего доступа к данным через UIActivityViewController". |
...
> [!Important]
> В пакете SDK для приложений Intune для iOS предусмотрено использование 256-битных ключей шифрования, если шифрование разрешено политиками защиты приложений. Чтобы разрешить общий доступ к защищенным данным, все приложения должны иметь текущую версию пакета SDK.
...
--       |  --   |   --       |  
AutoEnrollOnLaunch| Логический| Указывает, должно ли приложение автоматически регистрироваться при запуске, если приложение еще не зарегистрировано и если обнаружено существующее управляемое удостоверение. По умолчанию — NO. <br><br> Примечание. Если управляемое удостоверение не найдено или в кэше ADAL/MSAL отсутствует действительный токен для удостоверения, то попытка регистрации завершится сбоем без сообщения об ошибке и без запроса учетных данных (за исключением случая, когда параметр приложения MAMPolicyRequired имеет значение YES). |
MAMPolicyRequired| Логический| Указывает, будет ли блокироваться запуск приложения при отсутствии политики защиты приложений Intune. По умолчанию — NO. <br><br> Примечание. В магазин приложений нельзя отправить приложения, у которых параметр MAMPolicyRequired имеет значение YES. При установке значения YES в параметр MAMPolicyRequired также следует установить значение YES в параметр AutoEnrollOnLaunch. |
...
Дополнительные сведения о создании целевой политики конфигурации приложений MAM в iOS см. в разделе, посвященном целевой конфигурации MAM приложения, статьи [Использование политик конфигурации приложений Microsoft Intune для iOS](app-configuration-policies-use-ios.md).
...
В любой момент времени каждый поток имеет действующее удостоверение для задач, связанных с пользовательским интерфейсом, и задач файлов. Это удостоверение используется для определения применяемых политик (если таковые имеются). Если в качестве удостоверения задано значение "no identity" (без удостоверения) либо пользователь не является управляемым, никакие политики не применяются. На рисунках ниже показано, как определяются действующие удостоверения.
  ...
  ![Пакет Intune App SDK для iOS: процесс определения удостоверений](./media/ios-thread-identities.png)
...
Пакет SDK Intune для iOS 9.0.3 + поддерживает возможность добавления консоли диагностики в мобильное приложение для тестирования политик и ведения журнала ошибок. `IntuneMAMDiagnosticConsole.h`Определяет интерфейс `IntuneMAMDiagnosticConsole` класса, который разработчики могут использовать для вывода консоли диагностики Intune. Это позволяет конечным пользователям или разработчикам во время тестирования получать и совместно использовать журналы Intune для диагностики любых проблем, которые они могут иметь. Этот API является необязательным для интеграторов.
