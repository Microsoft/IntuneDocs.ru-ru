...
- Иногда в приложении может достигаться максимальный размер исполняемого файла Dalvik (DEX) из-за классов пакета SDK для Intune MAM, которые добавляются во время упаковки (необязательно). Файлы DEX являются частью компиляции приложения Android. Инструмент упаковки приложений Intune автоматически обрабатывает переполнение DEX файл во время упаковки для приложений на основе min API уровня 21 или более поздней версии (начиная с версии [v. 1.0.2501.1](https://github.com/msintuneappsdk/intune-app-wrapping-tool-android/releases)). Для приложений с помощью минимум уровень API 21 < будет оптимальным образом для увеличения минимального значения на уровне API, с помощью оболочки `-UseMinAPILevelForNativeMultiDex` флаг. Для клиентов, не удается повысить минимальный уровень API приложения доступны следующие решения DEX переполнения. В некоторых организациях для этого может потребоваться взаимодействие с теми, кто компилирует приложение (например, с группой по сборке приложений).
* Используйте ProGuard, чтобы исключить ссылки на неиспользуемый класс из основного файла DEX приложения.
* Для клиентов, использующих v3.1.0 или более поздней версии подключаемого модуля Android Gradle, отключите [D8 dexer](https://android-developers.googleblog.com/2018/04/android-studio-switching-to-d8-dexer.html).  
...
|**-UseMinAPILevelForNativeMultiDex**| (Необязательно) Используйте этот флаг для увеличения исходного Android приложения минимальный уровень API 21. Этот флаг запросит подтверждение, как будет ограничено, могут установить это приложение. Диалоговое окно подтверждения можно исключить путем добавления параметра «-подтверждение: $false» для своей команды PowerShell. Флаг должен использоваться только клиенты, использующие приложения с помощью min API < 21, не смогут успешно переносить из-за ошибки переполнения DEX. | |
