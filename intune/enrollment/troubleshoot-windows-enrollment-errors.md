...
Ошибка 80180003: "Произошла ошибка. У этого пользователя нет права на регистрацию. Вы можете повторить попытку или обратиться к системному администратору, используя код ошибки 80180003.
...
1. Войдите в [центр администрирования диспетчера конечных точек (Майкрософт)](https://go.microsoft.com/fwlink/?linkid=2109431).
2. Выберите **устройства** > **ограничения регистрации** > **умолчанию** (в разделе **ограничения на число устройств**) > **Свойства** > **изменить** (рядом с **предельным**числом устройств) > увеличьте **предельное число устройств** (до 15) > **Просмотр и сохранение**.    
...
2. Перейдите на вкладку **устройства** > **ограничения регистрации**, а затем выберите ограничение **по умолчанию** в разделе **ограничения по типу устройства**.    
...
- Другой пользователь уже зарегистрировал устройство в Intune или присоединил устройство к Azure AD. Чтобы определить, есть ли в этом случае, перейдите в раздел **параметры** > **учетные записи** > **Рабочий доступ**. Найдите сообщение, похожее на следующее: "другой пользователь в системе уже подключен к рабочей или учебной работе. Удалите это рабочее или учебное подключение и повторите попытку ".    
...
##### <a name="correct-the-mdm-terms-of-use-url"></a>Исправление URL-адреса условий использования MDM
  ...
  3. Выберите **восстановить URL-адреса MDM по умолчанию**, убедитесь, что в поле **URL-адрес условий использования MDM** задано значение **https://portal.manage.microsoft.com/TermsofUse.aspx** .    
  4. Выберите **Сохранить**.    


### <a name="something-went-wrong"></a>Произошла ошибка.

Ошибка 80180026: "Произошла ошибка. Подтвердите, что вы используете правильные данные для входа и что ваша организация использует эту функцию. Вы можете повторить попытку или обратиться к системному администратору, используя код ошибки 80180026. "
...
- Клиент Intune PC (агент Intune для компьютеров) установлен на компьютере с Windows 10.
...
2. Перейдите в раздел **Azure Active Directory** > **Mobility (MDM and MAM)**  > **Microsoft Intune**.    
3. Задайте для параметра **область пользователя MDM** значение **нет**, а затем нажмите кнопку **сохранить**.    
     
##### <a name="uninstall"></a>Удаление
Удалите агент клиента компьютера Intune с компьютера.    
...
2. Перейдите в раздел **администратор** > **Загрузка клиентского программного обеспечения**и нажмите кнопку **загрузить клиентское программное обеспечение**.    
...
1. В [центре администрирования Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431)выбирает **устройства** > **ограничения регистрации** > выберите ограничение по типу устройства.    
2. Выберите **свойства** > **изменить** (рядом с **параметрами платформы**) > **Разрешить** **Windows (MDM)** .    
3. Щелкните **проверить и сохранить**.    
...
- В Просмотр событий следующее событие регистрируется в **журналах приложений и служб/Microsoft/Windows/девицеманажемент-Enterprise-диагностика-поставщик/администратор**:   
    ...
    ```
**Причина**. Верно любое из условий ниже. 
- Имя участника-пользователя содержит непроверенный или не поддерживающий маршрутизацию домен, например Local (например, joe@contoso.local).    
...
#### <a name="resolution"></a>Решение
Убедитесь, что необходимый доступ к службам на основе Интернета для автопилота не заблокирован. Дополнительные сведения см. в статье [требования к сети для автопилота в Windows](https://docs.microsoft.com/windows/deployment/windows-autopilot/windows-autopilot-requirements-network).
...
Дополнительные сведения о развертывании устройства Windows в режиме киоска с помощью автопилота см. в разделе [развертывание киоска с использованием Windows автопилота](https://blogs.technet.microsoft.com/mniehaus/2018/06/07/deploying-a-kiosk-using-windows-autopilot/).
...
**Причина:** Наиболее распространенной причиной является использование гибридного присоединение к Azure AD и настройка функции назначить пользователя в профиле автопилота. Используя функцию назначить пользователя, вы выполняете присоединение к Azure AD на устройстве во время начального экрана входа, в котором устройство находится в состоянии, когда оно не может присоединиться к локальному домену. Таким образом, функцию назначить пользователя следует использовать только в стандартных сценариях автоматического развертывания присоединение к Azure AD.  Эта функция не должна использоваться в гибридных сценариях присоединений к Azure AD.

#### <a name="resolution"></a>Решение

1. В [центре администрирования Microsoft Endpoint Manager](https://go.microsoft.com/fwlink/?linkid=2109431)выберите > **устройства** > **Windows** > **устройства Windows**.
...
- [Поиск ошибок регистрации совместного управления](https://docs.microsoft.com/configmgr/comanage/how-to-monitor#enrollment-errors)
