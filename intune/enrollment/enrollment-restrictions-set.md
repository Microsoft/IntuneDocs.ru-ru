...
- число устройств;
- операционные системы и версии. Можно создать несколько ограничений и применить их к разным группам пользователей. Для ограничений можно задать [приоритет](#change-enrollment-restriction-priority).
...
3. Последовательно выберите **Регистрация устройства** > **Ограничения регистрации** > **Создать ограничение** > **Ограничение типа устройства**.
    ![Снимок экрана создания ограничения типа устройств](./media/enrollment-restrictions-set/create-device-type-restriction.png)
4. На странице **Основные сведения** задайте для ограничения **Имя** и **Описание** (необязательно).
5. Выберите **Далее**, чтобы перейти на страницу **Настройки платформы**.
6. В разделе **Платформа** выберите **Разрешить** для платформ, для которых необходимо разрешить это ограничение.
    ![Снимок экрана выбора настроек платформы](./media/enrollment-restrictions-set/choose-platform-settings.png)
7. В разделе **Версии** выберите минимальную и максимальную версии, которые должны поддерживать допустимые платформы. Ограничения версий применяются только к устройствам, зарегистрированным на корпоративном портале.
...
10. Выберите **Выбор групп для включения**, а затем используйте поле поиска, чтобы найти группы, которые необходимо включить в это ограничение. Это ограничение применяется только к группам, которым оно назначено. Если ограничение не назначено ни одной группе, оно не будет работать. Затем выберите **Выбрать**. 
    ![Снимок экрана выбора настроек платформы](./media/enrollment-restrictions-set/select-groups.png)
11. Выберите **Далее**, чтобы перейти на страницу **Проверка + создание**.
12. Выберите **Создать**, чтобы создать ограничение.
13. Будет создано ограничение с приоритетом выше заданного по умолчанию. Можно [изменить приоритет](#change-enrollment-restriction-priority).
...
1. Войдите на портал Azure.
2. Выберите **Больше служб**, выполните поиск по **Intune**, а затем выберите **Intune**.
3. Последовательно выберите **Регистрация устройства** > **Ограничения регистрации** > **Создать ограничение** > **Ограничение на число устройств**.
    ![Снимок экрана создания ограничения на число устройств](./media/enrollment-restrictions-set/create-device-limit-restriction.png)
4. На странице **Основные сведения** задайте для ограничения **Имя** и **Описание** (необязательно).
5. Выберите **Далее**, чтобы перейти на страницу **Предел устройств**.
6. Для параметра **Предел устройств** выберите максимальное количество устройств, которое может зарегистрировать пользователь.
...
1. Войдите на портал Azure.
2. Выберите **Больше служб**, выполните поиск по **Intune**, а затем выберите **Intune**.
3. Выберите **Регистрация устройства** > **Ограничения регистрации** > выберите ограничение, которое необходимо изменить > **Свойства**.
4. Выберите **Изменить** рядом с настройками, которые необходимо изменить.
5. На странице **Изменить** внесите необходимые изменения и перейдите на страницу **Просмотр + сохранение**, затем выберите **Сохранить**.
