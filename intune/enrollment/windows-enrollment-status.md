...
1. В [Intune](https://aka.ms/intuneportal) выберите **Регистрация устройства** > **Регистрация Windows** > **Страница состояния регистрации** > **Создать профиль**.
...
1. В Intune выберите **Регистрация устройства** > **Регистрация Windows** > **Страница состояния регистрации**.
...
- Сертификаты SCEP с политиками Windows Hello для бизнеса вызывают превышение времени ожидания, так как пользователь не может завершить настройку ПИН-кода Hello, чтобы разрешить завершение установки сертификата SCEP.  Обходного пути не существует. Исправление ожидается летом 2019 г. 
...
- Настройка управления приложениями в Защитнике Windows приводит к появлению запроса на перезагрузку при работе Autopilot. Настройка приложения (AppLocker CSP) в Защитнике Windows приводит к необходимости перезагрузки. Если настроена такая политика, устройство будет перезагружено при работе Autopilot. В настоящее время перезагрузку невозможно отменить или отложить.
