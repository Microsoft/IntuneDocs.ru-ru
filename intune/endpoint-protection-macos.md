  ...
  **Значение по умолчанию**: не настроено  
  ...
  - **Тип ключа восстановления**  
    Ключи восстановления *личных ключей* создаются для устройств. Настройте следующие параметры для личного ключа.  
     ...
     - **Расположение личного ключа восстановления** . Укажите короткое сообщение для пользователя, в котором объясняется, как можно получить свой личный ключ восстановления. Этот текст вставляется в сообщение, которое видит пользователь при включении FileVault.  
  ...
  - **Отложить FileVault до выхода** 
    > [!NOTE]
    > Поддержка FileVault ограничена до тех пор, пока Июль не завершит развертывание в течение нескольких дней. До завершения развертывания, если вы настроили FileVault, необходимо установить параметр *отложить FileVault, пока не* будет выполнен выход для **включения**.   

    FileVault не будет включаться, пока пользователь не выйдет из него. Локальный пользователь или учетная запись мобильного пользователя получат запрос на включение FileVault при выходе или при следующем входе.  
    - **Не настроено**.  
    - **Разрешить**  
    ...
    **Значение по умолчанию**: не настроено  
    ...
    - **Отключить запрос при выходе**  
      Запретите пользователю запрос на включение FileVault при выходе.  
      - **Не настроено**.  
      - **Разрешить**  
