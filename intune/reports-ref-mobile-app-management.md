---
title: "Управление мобильными приложениями (MAM) | Документация Майкрософт"
...
| IsDeleted |Указывает, обновлена ли запись приложения MAM. True — приложение MAM имеет новую запись с обновленными полями в этой таблице. False — последняя запись для этого приложения MAM. |Истина/ложь |
...
| IsDeleted |Указывает, обновлена ли запись этого экземпляра приложения MAM. True — этот экземпляр приложения MAM имеет новую запись с обновленными полями в этой таблице. False — последняя запись для этого экземпляра приложения MAM. |Истина/ложь |
...
| DeviceHealthName |Представляет состояние устройства. Not available — информация об этом устройстве отсутствует. Healthy — устройство не подвергалось взлому ОС. Unhealthy — устройство подвергалось взлому ОС. |Not Available Healthy Unhealthy |
...
| Свойство | Описание | Пример |
|---------|------------|--------|
| EffectivePolicyKey |Уникальный идентификатор эффективной политики MAM в хранилище данных |2 |
...
| Свойство | Описание | Пример |
|---------|------------|--------|
| PlatformKey |Уникальный идентификатор для платформы в хранилище данных — суррогатный ключ. |123 |
| Платформа |Уникальный идентификатор платформы — аналогичен PlatformKey, но является естественным ключом |123 |
| PlatformName |Имя платформы |Not Available None Windows IOS Android |
