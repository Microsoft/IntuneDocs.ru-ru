...
В Intune эти параметры можно использовать для настройки встроенного расширения Kerberos для Apple или расширения приложения единого входа, созданного вашей организацией. Расширение для приложения единого входа выполняет проверку подлинности пользователей. Эти параметры настраивают расширения для приложения единого входа по конкретным учетным данным, которые предназначены для потоков аутентификации запроса и подтверждения. Вы можете выбрать между расширением для учетных данных Kerberos, предоставляемых Apple, и расширением для общих учетных данных.
...
Дополнительные сведения о разработке расширения для приложения единого входа см. [на веб-сайте компании Apple](https://developer.apple.com/videos/play/tech-talks/301).

> [!NOTE]
> Функция **Расширение для приложения единого входа** отличается от функции **Единый вход**:
>
> - Параметры **расширения для приложения единого входа** применяются к iPadOS 13.0 (и более поздних версий) и iOS 13.0 (и более поздних версий). Параметры **единого входа** применяются к iPadOS 13.0 (и более поздних версий) и iOS 7.0 (и более поздних версий).
> - **Расширение для приложения единого входа** обрабатывает проверку подлинности в операционной системе. В случае **единого входа** определенное приложение обрабатывает аутентификацию.
> - При использовании **расширения для приложения единого входа** пользователи входят в приложения и на веб-сайты в автоматическом режиме с помощью Face ID, Touch ID или ПИН-кода либо секретного кода Apple. При использовании **единого входа** пользователи входят в приложения и на веб-сайты с помощью другого приложения.
>
>    **Расширение для приложения единого входа** использует операционную систему компании Apple для проверки подлинности. Таким образом, оно обеспечивает лучшее взаимодействие с пользователем.
>
> - С точки зрения разработки **расширение для приложения единого входа** может использовать любой тип проверки подлинности единого входа. При использовании **единого входа** можно использовать только проверку подлинности единого входа Kerberos.  
...
Добавьте пользовательское изображение с расширением .png, .jpg или .jpeg на защищенных устройствах с iOS. Например, в Intune можно добавить логотип компании на экран блокировки устройств.
