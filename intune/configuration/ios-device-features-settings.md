...
- **ИД расширения** (только учетные данные): введите идентификатор пакета, который определяет расширение приложения единого входа, например `com.apple.extensiblesso`.
- **Идентификатор команды** (только учетные данные): введите идентификатор команды расширения приложения единого входа. Идентификатор группы — это строка, состоящая из 10 символов (цифр и букв), созданная Apple, например `ABCDE12345`. Идентификатор команды не требуется.
...
- **Сертификат PKINIT** (только Kerberos): **выберите** сертификат шифрования открытого ключа для первоначальной проверки подлинности (PKINIT), который можно использовать для продления учетных данных Kerberos без взаимодействия с пользователем. Сертификат должен быть сертификатом PKCS или SCEP, добавленным ранее в Intune.
