...
Эти параметры используют [CSP шаредпк](https://docs.microsoft.com/windows/client-management/mdm/sharedpc-csp).
...
- **Время ожидания сна (в секундах)** : введите время в секундах (0–18 000), которое должно истечь, прежде чем устройство перейдет в спящий режим. `0` означает, что устройство не находится в спящем режиме. Если не задать время, устройство перейдет в спящий режим через 3600 секунд (60 минут).
...
- **Быстрый первый вход**: выберите **включено** , чтобы пользователи могли быстро войти в систему. Если этот параметр **включен**, устройство автоматически подключает новые учетные записи без прав администратора Azure AD к предварительно настроенным локальным учетным записям-кандидатам. Выберите **отключено** , чтобы предотвратить быстрый вход в систему. Значение **Не настроено** оставляет этот параметр не управляемым с помощью Intune (по умолчанию) и не передает его в политики для управления на устройстве.

  [Проверка подлинности и Енаблефастфирстсигнин CSP](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-authentication#authentication-enablefastfirstsignin)
