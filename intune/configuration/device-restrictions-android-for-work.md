...
- **Изменения тома**: **Блокировка** запрещает пользователям изменять том устройства, а также выключает главный том. Если выбран вариант **Не настроено**, на устройстве можно использовать параметры громкости.
...
- **Окна уведомлений**. Если задано значение **Отключить**, окно уведомлений, включая всплывающие уведомления, входящие вызовы, исходящие вызовы, системные оповещения и системные ошибки, не будут отображаться на устройстве. Если задано значение **Не настроено**, используется значение операционной системы по умолчанию, которое, возможно, разрешит показывать уведомления.
- **Пропустить указания по первому использованию**: **включить** скрытие или пропуск предложений из приложений, которые пошагово проходят через учебники, или указания при запуске приложения. Если задано значение **Не настроено**, будет использоваться значение операционной системы по умолчанию, которое может разрешать отображение этих предложений при запуске приложения.
...
- **Несколько приложений**. Пользователям доступен ограниченный набор приложений на устройстве. При запуске устройства запускаются только добавленные вами приложения. Можно также добавлять веб-ссылки, которые пользователи могут открывать. При применении этой политики на начальном экране отображаются значки разрешенных приложений.
  ...
  - **Режим заставки экрана**: **включить** отображает экранную заставку на управляемом начальном экране, если устройство заблокировано или время ожидания истекло. **Не настроено** (по умолчанию) не отображает экранную заставку на управляемом начальном экране.
    ...
    - **Задать пользовательский образ заставки**: введите URL-адрес для ПОЛЬЗОВАТЕЛЬСКОГО формата PNG, JPG, JPEG, GIF, BMP, Вебп или икоимаже. Например, введите:
      ...
      > [!TIP]
      > Поддерживается любой URL-адрес файлового ресурса, который можно преобразовать в точечный рисунок.
...
- **Время блокировки экрана**: введите максимальное время, которое пользователь может задать до блокировки устройства. Например, если задать для этого параметра значение **10 минут**, пользователи смогут задать время от 15 секунд до 10 минут. Если задано значение **не настроено** (по умолчанию), Intune не изменяет и не управляет этим параметром.
...
- **Удаление пользователя**. Выберите **Блокировать**, чтобы запретить удаление пользователей. Значение **Не настроено** (по умолчанию) разрешает пользователям удалять новых пользователей с устройства.
- **Изменения учетной записи** (только для выделенных устройств): выберите **блок** , чтобы запретить пользователям изменять учетные записи. Значение **Не настроено** (по умолчанию) разрешает пользователям изменять учетные записи на устройстве.
...
- **Личные учетные записи Google**: **блокировать** запрещает пользователям добавлять свои персональные учетные записи Google на устройство. **Не настроено** (по умолчанию). позволяет пользователям добавлять свои персональные учетные записи Google.
...
- **Рекомендуемый Глобальный прокси-сервер**. Выберите **включить** , чтобы добавить глобальный прокси-сервер для устройств. При включении трафика HTTP и HTTPS, включая некоторые приложения на устройстве, используйте введенный прокси-сервер. Этот прокси-сервер является только рекомендацией. Некоторые приложения могут не использовать прокси-сервер. **Не настроено** (по умолчанию) не добавляет рекомендуемый Глобальный прокси-сервер.
  ...
  - **Автоматическая настройка прокси-сервера**: введите **URL-адрес PAC** для скрипта автонастройки прокси-сервера. Например, введите `https://proxy.contoso.com/proxy.pac`.
