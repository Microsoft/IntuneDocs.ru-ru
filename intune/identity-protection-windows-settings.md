  ...
  **Значение по умолчанию**: не настроено
  ...
  Если задано значение *включено*, доступны следующие параметры:
    ...
    - **Минимальная длина PIN-кода**  
     Укажите минимальную длину ПИН-код для устройств, для безопасного входа в систему. По умолчанию устройства Windows используются шесть символов, но этот параметр можно принудительно задать как минимум 4 до 127 символов. 
  
      **Значение по умолчанию:** *Не настроено*

    - **Максимальная длина PIN-кода**  
    Укажите максимальную длину ПИН-код для устройств, для безопасного входа в систему. По умолчанию устройства Windows используются шесть символов, но этот параметр можно принудительно задать как минимум 4 до 127 символов.  
  ...
  - **Срок действия ПИН-кода (в днях)**  
      Рекомендуется указывать срок действия ПИН-кода, по истечении которого пользователю необходимо сменить его. Устройств Windows по умолчанию — 41 день.

    **Значение по умолчанию**: "Не настроено"

  - **Вести журнал изменения ПИН-кодов**  
    Этот параметр используется для запрета повторного применения ПИН-кодов, которые уже использовались ранее. По умолчанию устройства Windows для предотвращения повторного использования последние пять ПИН-кодов.  

    **Значение по умолчанию**: "Не настроено"  

  - **Включить восстановление ПИН-кода**   
    Позволяет пользователю использовать Windows Hello для бизнеса ПИН-код восстановления службы. 
    
    - **Включить** — ПИН-код восстановления секрета, хранящихся на устройстве, и пользователь может изменить свой ПИН-код, при необходимости.  
    - **Отключенные** -секрет восстановления не создана или хранимой.
...
- **Использование ключей безопасности для входа в систему**  
  Этот параметр доступен для устройств под управлением Windows 10 версии 1903 или более поздней версии. Используйте его для управления поддержку использования ключей Windows Hello безопасности для входа в систему.  

  - **Включить** -пользователи могут использовать ключ безопасности Windows Hello, нацелены учетные данные входа для компьютеров с этой политикой. 
  - **Отключить** - ключей безопасности отключены, и пользователи не могут использовать их для входа на ПК.   
