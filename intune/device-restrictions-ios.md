...
- **Публикация данных об использовании.** Выберите **Заблокировать**, чтобы запретить устройству передавать диагностические данные и данные телеметрии в Apple. Если выбрать вариант **Не настроено** (по умолчанию), такие данные будет разрешено отправлять.
  - **Изменение настроек отправки диагностических данных (только в защищенном режиме)**: **блок** запрещает пользователю изменять отправки и приложение analytics параметров диагностики в **о диагностике и использовании**(устройство параметры). Если выбрать вариант **Не настроено** (по умолчанию), пользователь сможет изменять такие параметры для устройства.

    Данная функция применяется к:  
    - iOS 9.3.2 и более поздние версии

- **Снимки экрана**. Выберите **Блокировать**, чтобы запретить на устройстве снимки экрана и захват экрана. Если выбрать вариант **Не настроено** (по умолчанию), пользователь сможет сохранять содержимое экрана в виде изображения.
...
- **Экрана (только в защищенном режиме) время**: выберите **блок** чтобы пользователи не смогут задать свои собственные ограничения в экран времени (параметры устройства). **Не настроено** разрешает пользователю настраивать ограничения устройств (например, родительский контроль или содержимое и о конфиденциальности ограничения) на устройстве.

  Этот параметр был переименован из **Включение ограничений в настройках устройства**. Влияние этого изменения:  
  
  - iOS 11.4.1 и более ранние версии: при выборе значения **Блокировать** пользователям запрещается настраивать собственные ограничения в параметрах устройства. Это то же самое; и нет изменений для конечных пользователей.
  - iOS 12.0 и более поздних версий: **блок** пользователям запрещено настраиваться собственные **время экрана** в настройках устройства (Параметры > Общие > время экрана), включая ограничения содержимого и о конфиденциальности. Устройства обновлены до iOS 12.0 не увидите вкладку ограничения в настройках устройства больше (Параметры > Общие > Управление устройствами > профиль управления > ограничения). Эти параметры перенесены в раздел **Время работы экрана**.
...
- **Запретить ограниченный режим USB (только защищенный режим).** Выберите **Блокировать**, чтобы отключить ограниченный режим USB на защищенных устройствах. В ограниченном режиме USB запрещен обмен данными между USB-аксессуарами и устройством, если оно заблокировано на период более одного часа. **Не настроено** (по умолчанию) разрешает USB в ограниченном режиме.
...
- **Разрешить аудитории для блокировки в приложение и заблокировать устройство без запроса подтверждения (только в защищенном режиме)**: **включить** позволяет преподавателю блокировки приложений или блокировать устройство, с помощью приложения "класс" без запроса подтверждения учащегося. Блокировка приложения означает, что устройство можно только преподавателя доступа указанных приложений. **Не настроено** (по умолчанию) предотвращает блокировку приложений или устройств с помощью приложения "класс" без запроса подтверждения учащийся преподавателей. 

  Данная функция применяется к:  
  - iOS 11.0 и более поздние версии

- **Автоматически присоединены к занятиями без запроса подтверждения (только в защищенном режиме)**: **включить** автоматически позволяет учащимся возможность соединения класса, который является в приложения "класс" без запроса подтверждения преподаватель. **Не настроено** (по умолчанию) предлагает преподаватель, требуется класс, который находится в приложения "класс" join учащихся.
...
- **Ограничить отслеживание рекламы.** Выберите **Ограничить**, чтобы отключить рекламный идентификатор устройства. **Не настроено** (по умолчанию) отслеживает его включенным.
...
- **Изменение параметров eSIM (только в защищенном режиме)**: **блок** запрещает пользователям удалять или добавление сотовой связи плана eSIM на устройстве. Если выбрать вариант **Не настроено** (по умолчанию), пользователь сможет изменять такие параметры.

  Данная функция применяется к:  
  - iOS 12.1 и более поздние версии

- **Отложить обновления программного обеспечения (только в защищенном режиме)**: Если значение **не настроен** (по умолчанию), обновления программного обеспечения, отображаются на устройстве как Apple их выпуска. Например если обновление iOS получает выпущено компанией Apple в определенный день, затем обновления естественным образом появится на устройстве с датой выпуска.
  ...
  **Включить** позволяет задержать, если обновления программного обеспечения, которые отображаются на устройствах из 0 – 90 дней. Этот параметр не контролирует, когда обновления или не установлены. 
  ...
  - **Задержка видимостью обновлений программного обеспечения**: Введите значение от 0 – 90 дней. Когда задержка истекает, пользователи получают уведомление о том, что необходимо обновить ОС до самой ранней версии, которая была доступна на момент активации задержки.
    ...
    Например если iOS 12.a можно найти в **1 января**, и **отложить видимость** присваивается **5 дней**, затем iOS 12.a не показано как доступное обновление на устройствах конечных пользователей. На **шестой день** после выпуска, доступно обновление, что конечные пользователи могут установить его.
...
- **Требуется Touch ID и Face ID проверки подлинности пароль или кредитная карта сведения Автозаполнение (только в защищенном режиме)**: Если значение **требуют**, пользователи должны проходить проверку подлинности с помощью TouchID или FaceID перед пароли или кредитная карта сведения могут быть автоматически заполняться сведениями в Safari и другими приложениями. **Не настроено** (по умолчанию) позволяет пользователям контролировать эту функцию в настройках устройства.
...
- **Просмотр корпоративных документов в неуправляемых приложениях.** Выберите **Блокировать**, чтобы запретить просмотр некорпоративных документов в неуправляемых приложениях. Если выбрать вариант **Не настроено**, можно будет просматривать корпоративные документы в любом приложении. Предположим, вы намерены запретить пользователям сохранять файлы из приложения OneDrive в Dropbox. В таком случае укажите для этого параметра значение **Блокировать**. Когда устройство получит эту политику (например, после перезагрузки), пользователь больше не сможет сохранять файлы.
  - **Разрешить управляемым приложениям для записи в контакты неуправляемые учетные записи контактов**: Если значение **Разрешить**, пользователи могут добавлять или синхронизировать любое лицо Outlook контактные данные, включая бизнеса и корпоративным контактам на встроенное приложение контакты на устройстве. Если задано значение **Не настроено**, пользователи не могут добавлять контакты Outlook во встроенном приложении "Контакты" на устройстве.
...
- **Просмотр некорпоративных документов в корпоративных приложениях.** Выберите **Блокировать**, чтобы запретить просмотр некорпоративных документов в корпоративных приложениях. Если выбрать вариант **Не настроено**, можно будет просматривать любые документы в управляемых корпоративных приложениях.
  - **Позволяют неуправляемым приложениям для чтения из учетных записей управляемых контакты**: Если значение **Разрешить**, пользователи могут добавлять любое лицо iContacts контактные сведения о приложении в Outlook. Значение **Не настроено** запрещает чтение, в том числе удаление дубликатов, во встроенном приложении "Контакты" на устройстве.
...
- **Safari**: выберите пункт **Блокировать**, чтобы запретить использование браузера Safari на устройстве. Если выбрать вариант **Не настроено**, пользователи смогут использовать браузер Safari.
...
- **Изменения параметров плана мобильной связи (только в защищенном режиме)**: **блок** запрещает пользователям изменять любые параметры в плане сотовой связи. **Не настроено** (по умолчанию) позволяет пользователям вносить изменения.

  Данная функция применяется к:  
  - iOS 11.0 и более поздние версии

- **Личный хот-спот.** Выберите **Блокировать**, чтобы запретить использование устройства в качестве индивидуальной точки доступа. Этот параметр может быть несовместимым с предложениями некоторых операторов связи. **Не настроено** (по умолчанию) разрешает эту функцию.
...
- **Блокировать параметр вверх new находящихся поблизости устройств (только в защищенном режиме)**: **блок** блокирует вывод запроса для настройки новых устройств, которые расположены рядом. **Не настроено** (по умолчанию) разрешает приглашения для пользователей, для подключения к другим близлежащим устройствам Apple.
...
- **Block iCloud Document sync** (Блокировать синхронизацию документов в iCloud). Выберите **Не настроено**, чтобы разрешить синхронизацию документов и пар "ключ-значение" в хранилище iCloud. Если выбрать вариант **Блокировать**, синхронизация документов и данных в iCloud будет запрещена.
...
- **Приложение для работы в режиме киоска.** Выберите тип приложений, которые должны выполняться в режиме киоска. Доступны следующие параметры:
  ...
  - **Встроенное приложение**: введите [идентификатор пакета](#bundle-ids-for-built-in-ios-apps) (в этой статье) встроенного приложения.
...
- **Неотмеченные домены электронной почты** > **URL-адрес домена электронной почты**: добавить один или несколько URL-адреса в список. Когда пользователь получает сообщение электронной почты с адреса, принадлежащего любому другому домену, оно отмечается как ненадежное в почтовом приложении iOS.

- **Управляемые веб-домены** > **URL-адрес домена**; Добавьте один или несколько URL-адреса в список. Документы, скачиваемые из указанных здесь доменов, считаются управляемыми. Этот параметр применяется только к документам, загруженным с помощью браузера Safari.

- **Домены автозаполнения пароля Safari** > **URL-адрес домена**: добавить один или несколько URL-адреса в список. Пользователи могут сохранять только веб-пароли для URL-адресов из этого списка. Этот параметр применяется только к браузеру Safari и iOS 9.3 или устройствам более поздних версий в защищенном режиме. Если вы не укажете URL-адрес, пользователи смогут сохранять пароли для всех веб-сайтов.
...
Также можно ограничить функции и параметры устройства на [macOS](device-restrictions-macos.md) устройств.