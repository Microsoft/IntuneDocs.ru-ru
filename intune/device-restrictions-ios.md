---
title: Параметры ограничений Microsoft Intune для устройств iOS
titleSuffix: ''
description: Сведения о параметрах Intune, с помощью которых можно управлять параметрами и работой устройств iOS.
keywords: ''
...
В этой статье описаны параметры ограничений устройств в Microsoft Intune, которые можно настроить для устройств под управлением iOS.

## <a name="general"></a>Общие

-   **Предоставление доступа к данным об использовании.** Разрешает или запрещает устройству передавать диагностические данные и данные телеметрии в Apple.
...
-   **Пароль.** Пользователь должен ввести пароль для доступа к устройству.
    ...
    -   **Число неудачных попыток входа перед очисткой устройства**. Указывает число уникальных неудачных попыток ввода пароля перед очисткой устройства.
...
-   **Уведомления кошелька при заблокированном устройстве.** Разрешает пользователю получать доступ к приложению кошелька на заблокированном устройстве.
...
Пример: выполните поиск по запросу Microsoft Word для iPad. Используемый URL-адрес имеет вид https://itunes.apple.com/us/app/microsoft-word-for-ipad/id586447913?mt=8.
...
- **Скрытые приложения.** Список приложений, которые будут скрыты от пользователей. Пользователи будут видеть и не смогут запускать эти приложения.
- **Видимые приложения.** Список приложений, которые пользователи будут видеть и смогут запускать. Другие приложения не будут доступны для просмотра или запуска.
...
- **Правила использования передачи данных (только управляемые приложения).** Позволяет определять типы данных, которые могут использовать управляемые приложения в сети сотовой связи. Выберите один из следующих типов.
    - **Блокировать использование передачи данных**. Разрешает блокирование использования передачи данных для параметра **Все управляемые приложения** или **Выберите конкретные приложения**.
    - **Блокировать использование передачи данных в роуминге.** Разрешает блокирование использования передачи данных в роуминге для параметра **Все управляемые приложения** или **Выберите конкретные приложения**.
...
-   **Поиск определения слов (только с защитой).** Включает компонент iOS, позволяющий выделить слово и найти его определение.
...
-   **Синхронизация потока фотографий с iCloud.** Разрешает пользователям включить функцию **Мой фотопоток** на своем устройстве, позволяющую синхронизировать фотографии с iCloud и использовать их на всех устройствах.
-   **Зашифрованное резервное копирование.** Требует шифрования резервных копий устройств.
-   **Медиатека iCloud.** Позволяет пользователям сохранять фотографии и видео в облаке, значение **Нет** отключает медиатеку iCloud.   При установке значения **Нет** все фотографии, загруженные из медиатеки iCloud на устройство не полностью, удаляются.
...
-   **Приложение, выполняемое в режиме киоска**. Выберите параметр **Управляемое приложение**, чтобы выбрать приложение, добавленное в Intune, параметр **Приложение магазина**, чтобы указать URL-адрес приложения в магазине, или параметр **Встроенное приложение**, чтобы указать идентификатор пакета встроенного приложения. Дополнительные сведения см. в разделе [Справочник по идентификаторам наборов для встроенных приложений iOS](device-restrictions-ios.md#bundle-id-reference-for-built-in-ios-apps) и [Способы указания URL-адреса для приложения в магазине](device-restrictions-ios.md#how-to-specify-the-url-to-an-app-in-the-store-1).
...
В поле **URL-адрес домена электронной почты** добавьте один или несколько URL-адресов в список. Когда пользователь получает сообщение электронной почты с домена, отличного от настроенного вами, оно помечается как ненадежное в почтовом приложении iOS.


### <a name="managed-web-domains"></a>Управляемые веб-домены

В поле **URL-адрес веб-домена** добавьте один или несколько URL-адресов в список. Документы, загруженные с указанных вами доменов, считаются управляемыми. Этот параметр применяется только к документам, загруженным с помощью браузера Safari.
