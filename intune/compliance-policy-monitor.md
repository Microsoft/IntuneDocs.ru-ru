...
2. Выберите **Соответствие устройства** > **Обзор**. Открывается **панель мониторинга соответствия устройств**.
...
### <a name="device-compliance-status-report"></a>Отчет о состоянии соответствия устройств

На диаграмме отображаются состояния соответствия для всех зарегистрированных в Intune устройств. Состояния соответствия устройств хранятся в двух разных базах данных — Intune и Azure Active Directory. 
...
- **Льготный период**: для устройства предназначен один из параметров политики соответствия для устройств или несколько. При этом пользователь еще не применил политики. Это значит, что устройство не соответствует условиям, но для него действует период отсрочки, определенный администратором.

  - Дополнительные сведения см. в разделе [Действия для несоответствующих устройств](actions-for-noncompliance.md).

- **Не вычисляется**: начальное состояние для новых зарегистрированных устройств. Также для устройств, которым не назначены политики соответствия требованиям и на которых нет триггера для проверки на соответствие.
...
Появятся дополнительные сведения об устройствах в этом состоянии, включая платформу операционной системы, дату последней регистрации и многое другое. 
...
![Фильтрации по адресу электронной почты позволяет показать все устройства для конкретного пользователя](./media/compliance-policy-monitor/idc-3.png)
...
На диаграмме выберите устройство и используйте пункт **Соответствие устройства политике**:
...
Будут показаны более подробные сведения о параметрах политики соответствия, примененных на этом устройстве. При выборе конкретной политики программа показывает все параметры в политике.

### <a name="devices-without-compliance-policy"></a>Устройства без политики соответствия
В отчете **Соответствие устройства политике** > **Обзор** также указываются устройства, которым не назначены политики соответствия.
...
- С помощью параметра безопасности **Помечать устройства без назначенной политики соответствия как** очень важно указать устройства без политики соответствия. Затем им можно назначить по меньшей мере одну политику соответствия.
  ...
  Этот параметр безопасности можно настроить на портале Intune. Выберите пункты **Соответствие устройства политике** > **Параметры политики соответствия**. Задайте для параметра **Помечать устройства без назначенной политики соответствия как** значение **Соответствует** или **Не соответствует**. 
...
Отчет **Соответствие устройств требованиям** > **Соответствие политике** отображает политики и число соответствующих и не соответствующих им устройств. 
...
При выборе конкретной политики вы увидите **состояние соответствия**, **псевдоним электронной почты пользователя**, **модель устройства** и **расположение** для каждого устройства, которому назначена эта политика соответствия требованиям.

## <a name="setting-compliance-report"></a>Отчет о соответствии параметру

В отчете **Соответствие устройств требованиям** > **Соответствие параметру** для каждого параметра соответствия указано общее количество устройств в каждом состоянии соответствия. В нем отображаются все параметры соответствия политики из всех политик соответствия требованиям, платформы, на которых применяются параметры политики, и количество несоответствующих устройств.
...
2. Выберите политику, затем щелкните **Обзор**. В этом представлении отображаются следующие состояния назначения политики.
    ...
    - Ошибка: политики не удалось применить. Сообщения обычно отображаются с кодом ошибки, которая указывает на описание. 
