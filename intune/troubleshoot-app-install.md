...
## <a name="user-group-targeted-app-installation-does-not-reach-device"></a>Установка приложения действует не достигает устройства
При возникновении проблем с установкой приложения следует учитывать следующие действия:
- Если приложение не отображается на корпоративном портале, убедитесь, приложение развертывается с **доступно** намерения и что пользователь осуществляет доступ к корпоративному порталу с типом устройства, поддерживаемые приложением.
- Для устройств Windows BYOD пользователь должен добавить рабочую учетную запись на устройство.
- Проверьте, если он превышает предел устройства AAD:
  1. Перейдите к [параметры устройств Azure Active Directory](https://portal.azure.com/#blade/Microsoft_AAD_IAM/DevicesMenuBlade/DeviceSettings/menuId).
  2. Запишите значение, заданное для **максимально устройств на пользователя**.
  3. Перейдите к [пользователей Azure Active Directory](https://portal.azure.com/#blade/Microsoft_AAD_IAM/UsersManagementMenuBlade/AllUsers).
  4. Выберите соответствующего пользователя и нажмите кнопку **устройств**.
  5. Если пользователь превышает установленный предел, удалите все устаревшие записи, которые больше не требуются.
- Для устройств iOS DEP, убедитесь, что пользователь указан как **зарегистрировал пользователь** в колонке обзора устройств Intune. Если он показывает, н/д, затем разверните политики конфигурации для корпоративного портала Intune. Дополнительные сведения см. в разделе [Настройка приложения корпоративного портала](https://docs.microsoft.com/intune/app-configuration-policies-use-ios#configure-the-company-portal-app-to-support-ios-dep-devices).
...
- Хранимые журналы шифруются для защиты любых содержащихся в них персональных данных.
...
| Невозможно устанавливать приложения при отключении App Store (–2016330860).  | App Store должен быть включен для пользователя для установки приложения. |
| Не удалось найти лицензию VPP для приложения (–2016330859).  | Попробуйте отозвать и переназначить лицензию на приложение. |
| Невозможно устанавливать системные приложения с помощью вашего поставщика MDM (–2016330858). | Установка приложений, которые предварительно установлены в операционной системе iOS, не поддерживается. |
...
| Пользователь должен войти в App Store (–2016330855). | Пользователь должен входить в App Store перед установкой приложения. |
