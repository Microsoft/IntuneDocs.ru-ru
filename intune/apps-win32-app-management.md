...
2.  Добавьте полную строку команды установки для установки приложения. 
    ...
    В приведенной выше команде пакет `ApplicaitonName.exe` поддерживает аргумент команды `/quite`.<p> Чтобы узнать, какие аргументы поддерживает пакет приложения, обратитесь к поставщику приложения.
...
> [!NOTE]
> Функция зависимостей приложения будет доступна только после обновления агента управления Intune до версии 1904 (выше 1.18.120.0). Это может занять одну или две дополнительные недели после обновления версии службы до 1904.
